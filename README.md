...
| `run_context_sensitivity_workflow.py` | One-command local workflow for context-sensitivity evidence: builds paired logprobs, enriches verified-record sidecars, writes calibrated score-dump columns, verifies an artifact manifest, and can register the workflow report. |
| `build_verifier_signal_score_dump.py` | Converts verifier verified-record JSONL sidecars into standard score-dump columns such as `verifier_refuted`, `verifier_uncertainty`, `selfcheck_refute_rate`, `world_model_disagreement`, `world_model_conflict`, `world_model_conflict_delta`, and `context_sensitivity_*`, including state-transition prediction/conflict metadata, direct world-model ensemble agreement metadata, and optional paired evidence-context token logprob reports, enabling calibrated geometry fusion with external evidence. |
...
| `run_context_sensitivity_workflow.py` | 一条命令跑 context-sensitivity evidence：生成 paired logprob、enrich verified-record sidecar、写 calibrated score-dump 列、验证 artifact manifest，并可登记 workflow report。 |
| `build_verifier_signal_score_dump.py` | 将 verifier verified-record JSONL sidecar 转成标准 score-dump 列，例如 `verifier_refuted`、`verifier_uncertainty`、`selfcheck_refute_rate`、`world_model_disagreement`、`world_model_conflict`、`world_model_conflict_delta` 和 `context_sensitivity_*`，同时支持 state-transition prediction/conflict metadata、直接 world-model ensemble agreement metadata，以及可选 paired evidence-context token logprob report，让外部证据进入 calibrated geometry fusion。 |
