...
| `run_product_trace_replay_workflow.py` | Runs the raw-trace handoff end to end: redacted trace corpus, runtime-pair index, product runtime baseline, selector replay, recursive manifest, optional manifest verification, optional fingerprint cache reuse, and registry-ready workflow report. |
...
| `run_product_trace_replay_workflow.py` | 端到端执行 raw trace handoff：脱敏 trace corpus、runtime-pair index、产品 runtime baseline、selector replay、递归 manifest、可选 manifest verification、可选 fingerprint cache 复用和可注册 workflow report。 |
