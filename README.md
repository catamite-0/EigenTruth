...
| `run_product_runtime_baseline.py` | Aggregates saved `ProductTrace` JSON files into a request-runtime baseline with phase, cache, verifier-route, retrieval-use, staged-verification savings, optional `ProductRuntimeBudgetPolicy` gate metrics, optional JSONL sidecar storage for per-trace records, and optional trace-record cache reuse for repeated budget sweeps. |
...
| `run_product_trace_replay_workflow.py` | Runs the raw-trace handoff end to end: redacted trace corpus, runtime-pair index, product runtime baseline, selector replay, recursive manifest, optional manifest verification, optional manifest fingerprint cache reuse, optional runtime trace-record cache reuse, optional selector trace-input cache reuse, and registry-ready workflow report. |
...
| `run_product_runtime_baseline.py` | 聚合已保存的 `ProductTrace` JSON，输出请求级 runtime baseline：phase、cache、verifier route、retrieval 使用率、staged-verification 节省量、可选 `ProductRuntimeBudgetPolicy` gate、逐 trace record 的可选 JSONL sidecar 存储，以及重复 budget sweep 可复用的 trace-record cache。 |
...
| `run_product_trace_replay_workflow.py` | 端到端执行 raw trace handoff：脱敏 trace corpus、runtime-pair index、产品 runtime baseline、selector replay、递归 manifest、可选 manifest verification、可选 manifest fingerprint cache 复用、可选 runtime trace-record cache 复用、可选 selector trace-input cache 复用和可注册 workflow report。 |
