...
| `EvidenceAcquisitionConformalCalibrator` / `EvidenceAcquisitionCalibrationReport` / `audit_evidence_acquisition_risk` / `evidence_acquisition_record_from_trace` | Calibrates the whole post-acquisition answer/acquire/abstain policy score, compares a naive pre-acquisition threshold against the post-policy threshold, saves the post-policy threshold as a regular `CalibrationArtifact`, can extract labeled calibration rows from saved `ProductTrace` payloads or trace+feedback joins, and can alpha-spend feedback-prefix checks to flag when a deployed acquisition threshold needs recalibration. |
