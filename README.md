...
| `run_frontier_research_queue_bound_command_plan.py` | Parses or explicitly executes a bound frontier research queue command plan; default mode is dry-run, while real execution requires approved/reviewed bindings unless explicitly overridden for local debugging. |
...
| `run_frontier_research_queue_bound_command_plan.py` | 解析或显式执行 bound frontier research queue command plan；默认 dry-run，真实执行默认要求 approved/reviewed 绑定，除非本地调试时显式覆盖。 |
