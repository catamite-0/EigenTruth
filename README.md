...
| `plan_frontier_research_queue_input_collection.py` | Reads a bound frontier queue plan and lowers remaining source-backed inputs plus semantic-gap local artifact inputs into non-evidence collection/review requests, while surfacing unmapped command placeholders as review requests before execution. |
...
| `bind_frontier_research_queue_artifact_inputs.py` | Stages explicitly reviewed local artifact paths, such as semantic-gap verified-record and record-index sidecars, into frontier command bindings without executing commands or treating those artifacts as verifier evidence. |
...
| `plan_frontier_research_queue_input_collection.py` | Reads a bound frontier queue plan and lowers remaining source-backed numeric/temporal/mechanism inputs plus semantic-gap local artifact inputs into non-evidence collection requests, while keeping unmapped command placeholders as review requests. |
...
| `bind_frontier_research_queue_artifact_inputs.py` | 将显式审查通过的本地 artifact 路径（如 semantic-gap verified-record 与 record-index sidecar）写入 frontier command bindings，但不执行命令，也不把这些 artifact 当作 verifier evidence。 |
...
| `plan_frontier_research_queue_input_collection.py` | 从已绑定 frontier queue plan 中读取剩余 source-backed 输入和 semantic-gap 本地 artifact 输入，将 numeric/temporal/mechanism 与 verified-record/record-index 等缺口降为非证据 collection requests，并把无法归因的命令占位符保留为 review requests。 |
