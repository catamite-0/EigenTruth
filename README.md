...
| `ProductPromotionContract` / `ProductRuntimeEvidenceBundle` | Converts promoted release-candidate reports into product runtime, verifier-route, covariance quality-gate provenance, performance-bundle, performance score-dump cache, product-trace-replay workflow provenance, selfcheck-signal-fusion sample-quality/fusion provenance, feedback-policy workflow provenance including final-answer feedback rates, selector-replay, runtime-drift evidence, release-efficiency handoff evidence, budget-policy contracts, validated feedback-derived `ControlPolicyConfig`, and candidate control defaults consumed by the calibrated-control demo; `load_product_promotion_contract()` loads compact contracts, while `load_product_runtime_evidence_bundle()` lazily attaches optional promotion-contract manifest/registry provenance plus explicit selfcheck-signal-fusion workflow manifest verification and registry metadata for runtime traces. |
...
| `build_verifier_signal_score_dump.py` | Converts verifier verified-record JSONL sidecars into standard score-dump columns such as `verifier_refuted`, `verifier_uncertainty`, `selfcheck_refute_rate`, and `world_model_disagreement`, including both state-transition prediction metadata and direct world-model ensemble agreement metadata, enabling calibrated geometry fusion with external evidence. |
...
| `run_world_model_signal_calibration_workflow.py` | Runs a deterministic world-model correction loop end to end: state-transition fixture, world-model verifier sidecar, verifier-signal score dump, score/geometry fusion report, nested manifest verification, and optional local registry record. |
...
| `compare_release_candidates.py` | Combines registered readiness, route, optional required route-baselines including robustness evidence, performance-baseline, product-trace-replay workflow file or registry key, selfcheck-signal-fusion workflow file or registry key, feedback-policy workflow file or registry key, selector-replay, product-runtime-drift, release-efficiency, and adapter-family evidence into one fail-closed release candidate with runtime flags, verifier route, quality, runtime cost, covariance quality-drop gate, performance evidence bundle/cache gates, optional performance trend gates against an explicit prior baseline, selfcheck sample-quality/fusion gates, audit-route budgets, retrieval stress-control gates, strict-audit adapter-family profile support, named release-policy profiles such as `strict_structured_fact` and `frontier_audit`, validated feedback-policy config/control-default provenance, action-level and final-answer feedback safety metrics, runtime drift provenance, release-efficiency profile handoff, and latency/balanced/audit profiles. |
| `run_release_candidate_registry_workflow.py` | Runs the release-candidate gate, writes a manifest covering the candidate report plus selected readiness/route/performance/product-trace-replay/selfcheck-signal-fusion/feedback-policy/selector/runtime-drift/release-efficiency manifests and optional required route/adapter-family reports, supports named release policy profiles such as `strict_structured_fact` and `frontier_audit`, can explicitly require canonical plus paraphrase `structured_fact` robustness evidence, recursively verifies it, and registers the final candidate with runtime-profile, covariance quality gate, performance-baseline, performance evidence bundle readiness/cost/cache/trend, product-trace-replay, selfcheck signal-fusion workflow status/report/record/sample-quality metadata, feedback-policy workflow status/report/record/candidate-policy metadata, selector-replay, runtime-drift, release-efficiency, route-budget, release-policy profile, structured-fact robustness, and adapter-family metadata; compare, manifest-build, promotion verification, and workflow-report writes emit phase timing, compare/manifest/promotion share fingerprint and JSON artifact caches, `--fingerprint-cache` persists file fingerprints, `--artifact-json-cache` persists parsed JSON artifact payloads across repeated release checks, and `--manifest-fingerprint-workers` enables bounded parallel artifact fingerprinting for local release gates. |
...
| `ProductPromotionContract` / `ProductRuntimeEvidenceBundle` | 将已 promoted release-candidate report 转成产品 runtime、verifier route、covariance 质量 gate provenance、performance-bundle、performance score-dump cache、product-trace-replay workflow provenance、selfcheck-signal-fusion 样本质量/融合证据 provenance、包含 final-answer 反馈率的 feedback-policy workflow provenance、selector-replay、runtime-drift evidence、release-efficiency handoff evidence、budget policy contract、已校验的反馈派生 `ControlPolicyConfig` 和可被 calibrated-control demo 消费的候选 control defaults；`load_product_promotion_contract()` 加载 compact contract，`load_product_runtime_evidence_bundle()` 延迟附加可选 promotion-contract manifest/registry provenance，并可显式验证 selfcheck-signal-fusion workflow manifest、输出 registry metadata 到 runtime trace。 |
...
| `ClaimTriple` / `RuleBasedTripleExtractor` / `TripleEvidenceVerifier` | 增加无依赖 subject-predicate-object 审计路径：简单规则或调用方 metadata 生成 triple，再用可通过 source、metadata、claim id 或 subject anchor 绑定的证据片段逐槽检查 subject、predicate 和 object 覆盖，然后才输出更严格的 supported 结果。 |
...
| `build_verifier_signal_score_dump.py` | 将 verifier verified-record JSONL sidecar 转成标准 score-dump 列，例如 `verifier_refuted`、`verifier_uncertainty`、`selfcheck_refute_rate` 和 `world_model_disagreement`，同时支持 state-transition prediction metadata 和直接 world-model ensemble agreement metadata，让外部证据进入 calibrated geometry fusion。 |
...
| `run_world_model_signal_calibration_workflow.py` | 端到端运行确定性 world-model 校正闭环：state-transition fixture、world-model verifier sidecar、verifier-signal score dump、score/geometry fusion report、嵌套 manifest verification 和可选本地 registry record。 |
