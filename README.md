...
| `build_counterfactual_probe_handoff.py` | Bridges collection-corpus or unresolved-queue `counterfactual_probe` requests into label-free claim JSONL, generated counterfactual probe records, and pending-generation rows that can feed `eval_counterfactual_verification.py` without treating probes as verifier evidence. |
