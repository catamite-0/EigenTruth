...
| `summarize_unresolved_frontier_evidence.py` | Summarizes unresolved frontier evidence lanes into a read-only non-evidence closure artifact with lane statuses and next actions; the world-model lane reports rule-family closure, can ingest optional rule-input audit, rule-stub requeue, and requeued input-plan artifacts, and emits raw remaining numeric/temporal gaps, audit-adjusted entity/numeric/temporal required inputs, completed requeue count, and outstanding requeue count after subtracting promoted mechanism candidates. `compare_release_candidates.py` and `run_release_candidate_registry_workflow.py` can require the summary to promote with zero open next actions before `frontier_audit` release candidates pass. |
