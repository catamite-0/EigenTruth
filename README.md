...
| `run_feedback_policy_workflow.py` | Runs the complete post-hoc feedback-to-policy loop in one command: feedback join report, candidate control-policy recommendation, replay audit, top-level manifest, and optional registry record. |
...
| `run_feedback_policy_workflow.py` | 一条命令串起完整事后反馈到策略闭环：feedback join report、候选控制策略推荐、replay audit、顶层 manifest 和可选 registry record。 |
