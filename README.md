...
| `internal_eigenscore` / `spectral_effective_rank` / `lexical_semantic_entropy` | Computes INSIDE/EigenScore-style spectral diversity from hidden-state embeddings; benchmarks can optionally sample multiple continuations for `inside_eigenscore` and `inside_semantic_entropy`. |
...
| `internal_eigenscore` / `spectral_effective_rank` / `lexical_semantic_entropy` | 基于隐藏态嵌入计算 INSIDE/EigenScore 风格的谱分散度；benchmark 可选多采样续写生成 `inside_eigenscore` 和 `inside_semantic_entropy`。 |
