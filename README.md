...
| `build_citation_search_adapter_handoff.py` | Converts unresolved citation/search queue items into sanitized external-adapter requests, and can ingest returned search-result JSONL into an external retrieval corpus without copying labels, record ids, target ids, or model answers. |
...
| `run_wikipedia_citation_search_adapter.py` | Provides a dependency-free MediaWiki/Wikipedia external search command with query de-duplication, rate limiting, retries, snippets, and page extracts for the citation/search command boundary. |
...
| `build_citation_search_adapter_handoff.py` | 将 unresolved citation/search queue 转成去敏的外部 adapter 请求，并可把外部搜索结果 JSONL 回填为 external retrieval corpus；不会复制 label、record id、target id 或 model answer。 |
...
| `run_wikipedia_citation_search_adapter.py` | 提供无新增依赖的 MediaWiki/Wikipedia 外部搜索命令，带 query 去重、限速、重试、snippet 和 page extract，用于 citation/search 命令边界。 |
