...
redline comparisons, dataset-transfer gates such as `--min-dataset-count` /
`--required-dataset`, and release gates still need to pass.
...
`next_actions`. `frontier_audit` also enables
`--require-frontier-queue-execution-smoke`, so the summary must carry a passing,
manifest-verified `frontier_queue_execution_smoke.py` control-plane health
record before this coordination gate can close.
Promoted summaries are also carried into `ProductPromotionContract`, runtime
metrics, and bounded `ProductTrace` metadata so product handoff audits can see
the closure status, queue-execution smoke status, manifest path, and blocker
reasons without reparsing the release-candidate report.
...
| `uncertainty_escalation_report` / `uncertainty_escalation_policy_sweep` | Summarizes `run_verification_loop()` results with opt-in uncertainty escalation, including trigger rate, entity-sensitive trigger counts, route/retrieval usage, decision changes, and optional label-conditioned false-accept/selective-accuracy deltas. The policy sweep replays recorded loop results across candidate `min_confidence` thresholds, estimating the safety/coverage/trigger-rate frontier before a stronger verifier threshold is promoted. |
...
| `TraceProvenanceGraph` / `TraceProvenanceReport` / `TraceProvenanceNode` / `TraceProvenanceEdge` / `audit_trace_provenance` / `audit_evidence_graph_consistency` | Builds a dependency-free typed provenance graph over claims, verification results, local evidence strings, action requests/results, retrieval hits, source labels, and final-answer evidence. The structural audit flags supported claims without a local evidence path, missing or failed referenced action results, unknown final-answer claim references, and answered traces with claims but no final-answer evidence. The evidence-graph consistency audit adds a lightweight content check over supported-claim evidence: claim keywords, numbers, and entity-like tokens are compared against linked local evidence/retrieval hits so stale references, numeric drift, and cross-claim retrieval hits surface in `ProductTrace.to_bounded_dict()` and `product_runtime_metrics()` without pretending to be semantic entailment; the same counters can be gated in product-runtime drift and required as part of release provenance evidence. |
...
| `WorldModelGuardedActionExecutor` / `audit_world_model_action_gate` | Simulates planned actions through a world-model adapter before executor dispatch, checks predicted-state postconditions, fails closed on low confidence, low agreement, no matching rule, or refuted postconditions, and returns a trace-ready correction request without executing the wrapped action. `ProductTrace.world_model_action_gate_summary()` and `product_runtime_metrics()` expose gate coverage, pass/block rates, and failure reasons; `run_product_runtime_baseline.py` and `compare_product_runtime_baselines.py` can aggregate and gate those rates in drift reports. |
...
| `EvidenceQualityPolicy` / `EvidenceQualitySummary` / `assess_evidence_quality` / `summarize_evidence_quality` | Applies reusable freshness and provenance checks to evidence snippets, including source requirements, trusted-source matching, timestamp age, and time-sensitive claim scoping; summaries expose pass/fail counts, reason counts, and per-document assessments for trace, retrieval, later calibration, and runtime observability use. |
| `RetrievalActionExecutor` / `InMemoryRetriever` / `SQLiteFTSRetriever` / `HTTPJSONRetriever` / `ProvenanceFilteredRetriever` | Provides dependency-free retrieval executor shells for unsupported-claim evidence gathering, from local snippets or SQLite FTS to caller-provided HTTP JSON search services; retrieval adapter errors fail closed into structured `ActionResult` payloads, provenance filters can require source, source prefixes, score floors, metadata tags, and per-source caps before hits become verifier evidence, and optional evidence-quality policies add query-level plus overall `evidence_quality` summaries to action results that flow into `ProductTrace` bounded summaries and `product_runtime_metrics()`. |
...
| `frontier_queue_execution_smoke.py` | Builds a no-model frontier queue execution fixture that covers source-family seed staging plus command-plan/scaffold/staged-bindings/review/dry-run artifacts, verifies a manifest, and registers the smoke report without executing child frontier workflows. |
...
| `summarize_unresolved_frontier_evidence.py` | Summarizes unresolved frontier evidence lanes into a read-only non-evidence closure artifact with lane statuses and next actions; the citation lane now preserves blocked query-sweep failure/recommended-action counters, the world-model lane reports rule-family closure, can ingest optional rule-input audit, rule-stub requeue, requeued input-plan artifacts, input-fill result rollups, and multiple rule-promotion reports, the semantic-gap lane can ingest reviewed covered-fact route workflow reports, and the frontier queue execution lane can ingest input-binding audits, staged command bindings, command-binding review, and bound-command run reports before deciding whether seed inputs need staging, command execution is reviewed, dry-run only, blocked, or closed. If scoped covered-fact route evidence already promotes but covers only part of the unresolved target queue, the summary emits `expand_scoped_covered_fact_alignment` with coverage gap/rate metadata instead of falling back to broad citation tuning. Adapter-ready fill rollups surface a next action for deterministic rule-adapter/promotion replay without treating the rollup as verifier evidence. It can also record `frontier_queue_execution_smoke.py` as a control-plane health signal, including recursive smoke-manifest verification, without treating the smoke as closure evidence. `compare_release_candidates.py` and `run_release_candidate_registry_workflow.py` can require the summary to promote with zero open next actions before `frontier_audit` release candidates pass. |
| `plan_frontier_research_queue_commands.py` | Converts a refreshed frontier status report, evidence-gap plan, citation-binding evidence collection plan, source-family catalog collection plan, source-family structured-QA lane rerun queue, or unresolved frontier evidence summary into a non-executing command plan with action provenance, queue lifecycle/source-alignment metadata, command templates, placeholder counts, required inputs, planned outputs, optional `--only-active-research-queue` filtering for closed/superseded status queues, and optional manifest/registry metadata. Unresolved summaries lower semantic-gap covered-fact review or scoped covered-fact alignment expansion, blocked citation alignment, partial/audit-requeued world-model rule lanes, adapter-ready fill-rollup rule promotion, source-family URL seed staging, and frontier queue review/re-bind/execute/repair lanes into reviewable command templates without filling evidence values; citation alignment templates use query-sweep diagnostics to vary query modes, target routes, retrieval/verifier thresholds, and source-family coverage audit/collection follow-ups. Citation-binding collection plans first lower source-backed reject lanes through `build_citation_binding_source_family_tasks.py`, then stage adapter templates for supported official/statistical/scholarly/news/domain source families. Source-family collection plans lower ready provider families into Crossref, OpenAlex, World Bank, GDELT, seeded URL, or official-site catalog adapter templates, while structured-QA lane rerun queues lower reviewed lane-batch commands into the same binding/review/run control plane. |
...
| `plan_frontier_research_queue_input_collection.py` | Reads a bound frontier queue plan and lowers remaining source-backed inputs plus semantic-gap and frontier control-plane local artifact inputs into non-evidence collection/review requests, while surfacing unmapped command placeholders as review requests before execution. |
| `scaffold_frontier_research_queue_input_bindings.py` | Expands frontier input collection requests into editable source-backed binding JSONL skeletons and downstream fill-command hints without filling evidence values or approving review; source-family URL seed requests expand collection tasks into blank `source-family-url-seeds.jsonl` rows for official/news/domain adapters. |
| `audit_frontier_research_queue_input_bindings.py` | Audits edited source-backed binding sidecars before fill or seed-adapter execution, checking review status, required citation/value/URL fields, non-evidence markers, reserved-field leakage, numeric subject-binding resolution, and source-family URL seed readiness without executing child workflows. |
| `bind_frontier_research_queue_seed_inputs.py` | Stages audit-ready source-family URL seed sidecars back into frontier command bindings by replacing `--seeds ...` placeholders, while keeping bindings non-evidence and still requiring normal command-binding review before adapter execution. |
| `run_frontier_research_queue_input_fill_commands.py` | Dry-runs or explicitly executes only audit-ready frontier input-fill commands, checks planned output materialization, and records a non-evidence execution report before adapter/promotion steps. |
| `rollup_frontier_research_queue_input_fill_results.py` | Combines materialized frontier input-fill outputs into one adapter-ready rule-input JSONL sidecar, preserving unfilled tasks and downstream adapter command hints without executing adapter or promotion gates. |
| `run_frontier_research_queue_rule_adapter_promotion_workflow.py` | Dry-runs or explicitly executes the deterministic rule-authoring adapter plus rule-candidate promotion gate from a fill-result rollup, fail-closing when the rollup, rule stubs, or combined rule-input sidecar are not adapter-ready; optional `--build-handoff` and `--build-evidence-bundle` write ProductTrace/action-result and release-gate bundle artifacts only after upstream gates pass. |
...
| `eval_uncertainty_escalation.py` | Reads saved `VerificationLoopResult.to_dict()` JSON/JSONL records and reports uncertainty-escalation trigger rates, retrieval evidence rates, decision transitions, optional label-conditioned false-accept deltas, and optional `--sweep-min-confidence` policy sweeps over the recorded stronger-verifier outcomes. |
...
| `enrich_evidence_alignment_sidecar.py` | Joins local citation/search evidence JSON or JSONL records onto verifier sidecars as `EvidenceAlignmentVerifier` results, giving external retrieval adapters a dependency-free route into calibrated `evidence_alignment_*` score-dump signals. |
...
| `build_verifier_signal_score_dump.py` | Converts verifier verified-record JSONL sidecars into standard score-dump columns such as `verifier_refuted`, `verifier_uncertainty`, `selfcheck_refute_rate`, `fact_selfcheck_refute_rate`, `fact_selfcheck_uncovered_rate`, `evidence_alignment_*`, `perturbation_conflict_rate`, `perturbation_high_confidence_conflict_rate`, `world_model_disagreement`, `world_model_conflict`, `world_model_conflict_delta`, and `context_sensitivity_*`, including state-transition prediction/conflict metadata, fact-level selfcheck triple reports, claim-to-evidence alignment reports, prompt-perturbation consistency reports, direct world-model ensemble agreement metadata, and optional paired evidence-context token logprob reports, enabling calibrated geometry fusion with external evidence. |
...
| `run_retrieval_semantic_gap_review_workflow.py` | Chains retrieval semantic-gap handoff, alignment fact-review corpus building, deterministic rule review, reviewed-source promotion gating, and optional covered-fact QA route audit into one scoped workflow. |
| `bind_frontier_research_queue_artifact_inputs.py` | Stages explicitly reviewed local artifact paths, such as semantic-gap verified-record/record-index sidecars and frontier command-plan/bindings/review-decision artifacts, into frontier command bindings without executing commands or treating those artifacts as verifier evidence. |
| `review_frontier_research_queue_command_bindings.py` | Audits filled frontier command bindings and writes an approved bindings sidecar only for mechanically clean entries with explicit reviewer approval, keeping execution review reproducible and fail-closed. |
...
| `run_citation_search_evidence_workflow.py` | Runs returned citation/search adapter JSONL through handoff ingestion, adapter request-coverage gating, external-corpus provenance audit, blind-spot query sweep, configurable target-route gating, and optional controlled-vs-external comparison before any route promotion; blocked query sweeps now emit structured failure diagnostics for no-hit, route-selection, blind-refute-coverage, and false-alarm failures. |
| `summarize_citation_binding_audit_failures.py` | Turns claim-specific citation binding audit rejects into a read-only evidence-quality review with issue buckets, question-type counts, examples, and next-action recommendations such as collecting numeric/statistical evidence, causal/procedural evidence, timestamped sources, or claim-specific evidence spans; citation evidence workflows write this review automatically when `--audit-source-bindings` is enabled. Binding audits also record claim-specific evidence spans for accepted docs, while replacing bound-corpus text with those spans is an explicit `--clip-accepted-evidence-spans` / `--clip-binding-evidence-spans` experiment rather than the default. |
| `plan_citation_binding_evidence_collection.py` | Converts rejected citation binding rows into non-evidence collection requests split by gap lane, including numeric/statistical, temporal, causal/procedural, role/location, source-family, provenance repair, claim-specific span, and alignment-review tasks. `run_citation_search_evidence_workflow.py --audit-source-bindings` now writes this plan and JSONL request sidecar automatically so blocked evidence-quality work can feed later adapters without weakening gates. |
| `build_citation_binding_source_family_tasks.py` | Bridges source-collectable lanes from a citation-binding evidence collection plan into the existing `source_family_catalog_collection_plan` task schema, skipping review-only lanes and unsupported source families while preserving non-evidence provenance for downstream official/statistical/scholarly/news/domain adapters. |
...
| `requeue_world_model_rule_stubs_from_audit.py` | Applies rule-input audit requeue suggestions back to sanitized rule stubs, producing corrected non-evidence stubs plus a requeue report/manifest for the existing rule-authoring adapter and unresolved-closure summary. |
...
| `rollup_citation_search_batch_evidence.py` | Rolls citation/source-family child workflow reports into one release-auditable batch report, preserving expected/observed batch coverage, child manifest checks, child adapter-gate status/coverage counts, provenance/query-sweep/comparison evidence aggregates, query-sweep failure-diagnostic counts, and fail-closed blocker reasons. |
...
| `compare_claim_factuality_probe_workflows.py` | Aggregates compact claim factuality probe workflow reports across runs/models/datasets, applies ready/manifest/model-count/dataset-count/required-dataset/AUROC/conformal/text-redline margin gates, writes a leaderboard-style comparison report, and can emit a manifest plus optional registry records. |
...
| `compare_product_runtime_baselines.py` | Compares current product runtime baselines against a file or registered baseline, emitting fail-closed drift gates over latency, route cost, retrieval use, cache reuse, verifier skip rate, trace-level world-model participation/coverage/conflict/low-agreement/trace-gap drift, world-model action-gate coverage/pass/block/postcondition drift, trace-level context-sensitivity participation/coverage/flagged/trace-gap/ratio drift, trace-level counterfactual-robustness participation/coverage/pass/flip-success/false-invariance/trace-gap drift, claim-risk localization coverage and entity-exposure/high-risk-count drift, promotion-contract/triple-extraction evidence coverage and quality drift, pre-generation probe comparison/redline handoff drift, counterfactual verifier-audit handoff drift, fact-selfcheck gate coverage/manifest/pass/run-quality handoff drift, frontier release-evidence citation/source-family batch, child adapter-gate count metadata, rerun-rollup track drift, optional unresolved-frontier closure/queue-smoke summary drift, promotion-contract evidence-handoff coverage/manifest/metric-gap/group-status drift, covered-fact per-property rollup drift, product-trace action-audit/action-execution gate rate drift, action-receipt coverage/missing/invalid/fingerprint-mismatch/unsigned receipt drift, evidence-quality coverage/pass/failure/stale/untrusted/missing-source drift, trajectory-audit factual/referential/logical/procedural/scope drift, trace-provenance coverage/support-reference/missing-evidence drift, citation-integrity coverage/mismatch/unresolved/trace-gap drift, trace-level triple/slot-audit coverage, trace count, and an optional reusable runtime budget policy. |
...
`--require-product-runtime-drift-citation-integrity-evidence`, and
`--require-product-runtime-drift-metacognition-evidence`, so strict
frontier releases must preserve claim-factuality probe comparison evidence,
receipt coverage, explicit claim-to-receipt support, graph provenance, and
citation plus metacognition evidence through the
...
`--require-product-runtime-drift-fact-selfcheck-gate-evidence` is an opt-in
release-candidate hardening flag: it fails closed unless product-runtime drift
contains the fact-selfcheck signal-fusion gate coverage, manifest, pass, run,
and triple-count metrics, while remaining outside `frontier_audit` defaults.
Release candidates can also explicitly require the world-model guarded-action
runtime-drift evidence with
`--require-product-runtime-drift-world-model-action-gate-evidence`; this is an
optional promotion hardening flag and is not a `frontier_audit` default yet.
...
| `plan_frontier_research_queue_input_collection.py` | Reads a bound frontier queue plan and lowers remaining source-backed numeric/temporal/mechanism inputs plus semantic-gap and frontier control-plane local artifact inputs into non-evidence collection requests, while keeping unmapped command placeholders as review requests. |
| `scaffold_frontier_research_queue_input_bindings.py` | Reads that input collection plan and writes editable source-backed numeric/temporal/subject/mechanism binding sidecar skeletons, expanding rule-input tasks when their JSONL path is available; it also expands source-family collection tasks into blank URL seed skeletons for `--seeds` adapter inputs. |
| `audit_frontier_research_queue_input_bindings.py` | Checks edited source-backed binding sidecars before rule-input fill or seed-adapter execution, surfacing ready/blocked rows and fail-closed reasons without treating sidecars as verifier evidence. |
| `bind_frontier_research_queue_seed_inputs.py` | Reads an input-binding audit and stages ready `source-family-url-seeds.jsonl` sidecar paths into command bindings, replacing seed placeholders without approving or executing the source-family adapter commands. |
| `run_frontier_research_queue_input_fill_commands.py` | Consumes the binding audit and dry-runs or executes only ready fill commands, with planned-output checks and optional registry/manifest metadata. |
| `rollup_frontier_research_queue_input_fill_results.py` | Rolls up successful fill reports into a single combined rule-input sidecar for the rule-authoring adapter, while keeping unfilled rows queued. |
| `run_frontier_research_queue_rule_adapter_promotion_workflow.py` | Connects a fill-result rollup to rule-adapter execution, candidate promotion, optional ProductTrace/action-result handoff, and optional mechanism handoff evidence bundle; default mode is plan-only, while `--execute` runs local gates and writes manifest/registry metadata. |
...
| `uncertainty_escalation_report` / `uncertainty_escalation_policy_sweep` | 汇总带 uncertainty escalation 的 `run_verification_loop()` 结果，输出触发率、实体敏感触发计数、route/retrieval 使用率、决策迁移，以及可选标签条件下的 false-accept 和 selective-accuracy 变化；policy sweep 会在已记录的强验证结果上重放候选 `min_confidence` 阈值，给出 safety / coverage / trigger-rate 前沿。 |
...
| `WorldModelGuardedActionExecutor` / `audit_world_model_action_gate` | 在 executor dispatch 前用 world-model adapter 模拟 planned action，检查预测状态 postcondition，并在低置信、低一致性、无匹配规则或 postcondition 被 refute 时 fail closed，返回可进入 trace 的 correction request 且不执行被包装 action；`ProductTrace.world_model_action_gate_summary()` 和 `product_runtime_metrics()` 会暴露 gate 覆盖率、通过/阻断率与失败原因。 |
...
| `EvidenceAlignmentVerifier` / `audit_evidence_alignment` | 将 claim 与本地 evidence/retrieval/citation hit 做无依赖 slot-level 对齐审计，输出关键词覆盖、数字 recall、实体 recall、引用覆盖率、否定冲突和缺失槽位；数字缺失、否定冲突或强制 citation evidence 缺失会 fail closed，弱关键词/实体覆盖保持为 evidence-insufficient 诊断，方便后续 learned entailment 或外部 search adapter 接入。 |
...
| `EvidenceQualityPolicy` / `EvidenceQualitySummary` / `assess_evidence_quality` / `summarize_evidence_quality` | 对证据片段执行可复用的新鲜度和 provenance 检查，包括 source 要求、trusted-source 匹配、timestamp age 和时间敏感 claim 作用域；summary 会输出 pass/fail 计数、reason counts 和逐证据 assessment，供 trace、retrieval、后续 calibration 和 runtime observability 使用。 |
| `RetrievalActionExecutor` / `InMemoryRetriever` / `SQLiteFTSRetriever` / `HTTPJSONRetriever` / `ProvenanceFilteredRetriever` | 为 unsupported claim 的取证流程提供无依赖 retrieval executor shell，覆盖本地片段、SQLite FTS 和调用方提供的 HTTP JSON search service；retrieval adapter 异常会 fail closed 成结构化 `ActionResult`，provenance filter 可在 hit 进入 verifier evidence 前要求来源、来源前缀、分数下限、metadata 标签和每来源上限；可选 evidence-quality policy 会把 query-level 与 overall `evidence_quality` summary 写入 action result，并进入 `ProductTrace` bounded summaries 与 `product_runtime_metrics()`。 |
...
| `eval_uncertainty_escalation.py` | 读取已保存的 `VerificationLoopResult.to_dict()` JSON/JSONL 记录，报告 uncertainty escalation 触发率、retrieval evidence rate、决策迁移和可选标签条件下的 false-accept 变化；可用 `--sweep-min-confidence` 在同一 sidecar 上生成策略阈值 sweep。 |
...
| `enrich_evidence_alignment_sidecar.py` | 将本地 citation/search evidence JSON 或 JSONL 记录合并进 verifier sidecar，写成 `EvidenceAlignmentVerifier` 结果，让外部检索适配器以无依赖方式进入 calibrated `evidence_alignment_*` score-dump 信号。 |
...
| `build_verifier_signal_score_dump.py` | 将 verifier verified-record JSONL sidecar 转成标准 score-dump 列，例如 `verifier_refuted`、`verifier_uncertainty`、`selfcheck_refute_rate`、`fact_selfcheck_refute_rate`、`fact_selfcheck_uncovered_rate`、`evidence_alignment_*`、`perturbation_conflict_rate`、`perturbation_high_confidence_conflict_rate`、`world_model_disagreement`、`world_model_conflict`、`world_model_conflict_delta` 和 `context_sensitivity_*`，同时支持 state-transition prediction/conflict metadata、fact-level selfcheck triple reports、claim-to-evidence alignment reports、prompt-perturbation consistency reports、直接 world-model ensemble agreement metadata，以及可选 paired evidence-context token logprob report，让外部证据进入 calibrated geometry fusion。 |
...
| `run_retrieval_semantic_gap_review_workflow.py` | 将 retrieval semantic-gap handoff、alignment fact-review corpus、确定性 rule review、reviewed-source promotion gate 和可选 covered-fact QA route audit 串成一个 scoped 闭环。 |
| `bind_frontier_research_queue_artifact_inputs.py` | 将显式审查通过的本地 artifact 路径（如 semantic-gap verified-record/record-index sidecar，以及 frontier command-plan、bindings、review-decision artifact）写入 frontier command bindings，但不执行命令，也不把这些 artifact 当作 verifier evidence。 |
| `review_frontier_research_queue_command_bindings.py` | 审计已填充的 frontier command bindings，并只给机械检查通过且有显式 reviewer approval 的 entry 输出 approved bindings sidecar，让执行审查可复现且 fail-closed。 |
...
| `run_citation_search_evidence_workflow.py` | 将 citation/search adapter 回填 JSONL 串过 handoff ingestion、adapter request 覆盖率 gate、external-corpus provenance audit、可配置目标路由的 blind-spot query sweep 和可选 controlled-vs-external comparison，再决定能否进入 route promotion；blocked query sweep 会输出 no-hit、route-selection、blind-refute 覆盖不足和 false-alarm 过高等结构化失败诊断。 |
| `summarize_citation_binding_audit_failures.py` | 将 claim-specific citation binding audit 的拒绝记录整理成只读 evidence-quality review，包含 issue bucket、question type 计数、示例和下一步建议，例如采集 numeric/statistical evidence、causal/procedural evidence、timestamped source 或 claim-specific evidence span；启用 `--audit-source-bindings` 时 citation evidence workflow 会自动写出该 review。Binding audit 也会为 accepted doc 记录 claim-specific evidence span，但用这些 span 替换 bound-corpus text 是显式 `--clip-accepted-evidence-spans` / `--clip-binding-evidence-spans` 实验开关，不作为默认行为。 |
| `plan_citation_binding_evidence_collection.py` | 将 citation binding rejected rows 按 gap lane 转成非证据采集请求，包括 numeric/statistical、temporal、causal/procedural、role/location、source-family、provenance repair、claim-specific span 和 alignment-review 任务；`run_citation_search_evidence_workflow.py --audit-source-bindings` 会自动写出 plan 和 JSONL request sidecar，让 blocked evidence-quality 工作能进入后续 adapter，同时不放松任何 gate。 |
| `build_citation_binding_source_family_tasks.py` | 将 citation-binding evidence collection plan 中可由 source-family adapter 继续采集的 lane 桥接成既有 `source_family_catalog_collection_plan` task schema，跳过 review-only lane 和暂不支持的 source family，同时保留非证据 provenance，供 official/statistical/scholarly/news/domain adapter 消费。 |
...
| `rollup_citation_search_batch_evidence.py` | 将 citation/source-family 子 workflow 汇总为 release-auditable batch report，保留 expected/observed batch coverage、child manifest 校验、adapter-gate coverage/status、provenance/query-sweep/comparison 聚合指标、query-sweep 失败诊断计数和 fail-closed blocker reason。 |
...
| `compare_claim_factuality_probe_workflows.py` | 聚合多个 run/model/dataset 的紧凑 claim factuality probe workflow report，应用 ready/manifest/model-count/dataset-count/required-dataset/AUROC/conformal/text-redline margin gate，输出 leaderboard 风格 comparison report，并可写入 manifest 和可选 registry records。 |
...
| `frontier_queue_execution_smoke.py` | 生成无需模型的 frontier queue execution fixture，覆盖 source-family seed staging 与 command-plan/scaffold/staged-bindings/review/dry-run artifacts，验证 manifest 并登记 smoke report，但不执行子 frontier workflow。 |
...
| `summarize_unresolved_frontier_evidence.py` | 将 unresolved blind-spot queue、citation/source-family workflow、source-family coverage audit、semantic-gap review workflow、world-model rule-input/promotion gate、rule-stub requeue report、requeued input plan、input-fill result rollup 和 mechanism handoff bundle 聚合成只读 closure summary，输出 lane status、next actions、manifest/registry metadata；citation lane 会保留 blocked query-sweep 的失败原因和推荐动作计数，semantic-gap lane 可消费 reviewed covered-fact route 结果，并在 scoped covered-fact route 已 promote 但覆盖目标队列不完整时输出 `expand_scoped_covered_fact_alignment`、覆盖缺口和覆盖率，而不是退回宽泛 citation tuning；world-model lane 会扣除已 promoted 的 rule-family 与已重排任务，避免已闭合候选污染下一轮计划，并在 fill rollup 已 adapter-ready 但 promotion 尚未执行时给出 deterministic rule-adapter/promotion replay 下一步；frontier queue lane 还可记录 `frontier_queue_execution_smoke.py` 的控制面健康状态和递归 manifest 校验，但不会把 smoke 当作闭合证据；它不生成 verifier evidence，也不关闭 release gate。 |
| `plan_frontier_research_queue_commands.py` | 将刷新后的 frontier status report、evidence-gap plan、citation-binding evidence collection plan、source-family catalog collection plan、source-family structured-QA lane rerun queue 或 unresolved frontier evidence summary 转成不执行命令的 command plan，记录 action provenance、queue lifecycle/source-alignment metadata、命令模板、placeholder 数、required inputs、planned outputs，支持显式 `--only-active-research-queue` 过滤 closed/superseded status queue，并可写 manifest/registry metadata；unresolved summary 会把 semantic-gap covered-fact review 或 scoped covered-fact alignment expansion、blocked citation alignment、partial/audit-requeued world-model rule lane、adapter-ready fill-rollup rule promotion，以及 frontier queue review/re-bind/execute/repair lane 降成可审查命令模板，但不会填入证据值；citation alignment 模板会根据 query-sweep 诊断切换 query mode、target route、retrieval/verifier 阈值，并补 source-family coverage audit/collection 后续命令；citation-binding collection plan 会先经 `build_citation_binding_source_family_tasks.py` 降成 source-family collection tasks，再生成 supported provider adapter 模板；source-family collection plan 会按 ready provider family 生成 Crossref、OpenAlex、World Bank、GDELT、seeded URL 或 official-site catalog adapter 模板；structured-QA lane rerun queue 则把 lane batch 命令接入同一套 binding/review/run 控制面。 |
...
| `compare_product_runtime_baselines.py` | 将当前 product runtime baseline 与文件或 registry 中的基线比较，对 latency、route cost、retrieval 使用、cache 复用、verifier skip rate、world-model action-gate 覆盖/pass/block/postcondition 漂移、claim-risk localization 覆盖率与实体暴露/高风险计数漂移、promotion-contract / triple-extraction evidence 覆盖与质量漂移、pre-generation risk telemetry / probe comparison / redline handoff 漂移、counterfactual verifier-audit handoff 漂移、fact-selfcheck gate 覆盖/manifest/pass/run-quality handoff 漂移、frontier release-evidence citation/source-family batch 与 rerun-rollup track 漂移、可选 unresolved-frontier closure/queue-smoke summary 漂移、promotion-contract evidence-handoff 覆盖/manifest/metric-gap/group-status 漂移、covered-fact per-property rollup drift、product-trace action-audit / action-execution gate rate drift、action-receipt 覆盖率/缺失/无效/指纹不匹配/未签名 receipt 漂移、evidence-quality 覆盖/pass/failure/stale/untrusted/missing-source 漂移、trajectory-audit factual/referential/logical/procedural/scope drift、trace-provenance 覆盖/支持引用/缺失证据漂移、evidence-graph consistency 覆盖/一致率/数字缺失/cross-claim hit 漂移、citation-integrity 覆盖/mismatch/unresolved/trace-gap 漂移、claim/evidence alignment 参与/覆盖/alignment/misalignment/insufficient/citation-reference/issue/trace-gap 漂移、trace 级 triple/slot-audit 覆盖、trace 数量和可选复用 runtime budget policy 输出 fail-closed drift gate。 |
...
`--require-product-runtime-drift-citation-integrity-evidence`、以及
`--require-product-runtime-drift-metacognition-evidence`，因此严格
frontier release 必须把 claim factuality probe comparison 证据、action receipt
覆盖率、显式 claim-to-receipt 支撑证据、trace graph provenance 与 citation
metadata/metacognition 证据保留到 product-runtime drift handoff。
release candidate 也可以显式加
`--require-product-runtime-drift-world-model-action-gate-evidence`，要求
world-model guarded-action 的覆盖率、通过/阻断率和 postcondition failure
原因进入 runtime-drift evidence；这个开关目前仍是可选加固项，不是
`frontier_audit` 默认项。
`--require-product-runtime-drift-fact-selfcheck-gate-evidence` 也是可选
release-candidate 加固项，会要求 fact-selfcheck signal-fusion gate 的覆盖率、
manifest、通过率、运行数和 triple-count 指标进入 product-runtime drift，
但仍不作为 `frontier_audit` 默认要求。
`run_product_trace_replay_workflow.py` 可用
`--min-runtime-drift-world-model-action-gate-coverage-rate`、
`--min-runtime-drift-world-model-action-gate-pass-rate` 和对应
`--max-runtime-drift-world-model-action-gate-*-rate-increase` 参数直接生成这组
runtime-drift 证据。
...
| `requeue_world_model_rule_stubs_from_audit.py` | 将 audit requeue 建议应用回去敏 rule stub，生成修正后的非证据 rule-authoring stub、requeue report、manifest/registry metadata；closure summary 可读取该 report，将已执行 requeue 从 outstanding next actions 中扣除。 |
...
| `plan_frontier_research_queue_input_collection.py` | 从已绑定 frontier queue plan 中读取剩余 source-backed 输入、semantic-gap 本地 artifact 输入和 frontier control-plane artifact 输入，将 numeric/temporal/mechanism、verified-record/record-index、command-plan/bindings/review-decision 等缺口降为非证据 collection requests，并把无法归因的命令占位符保留为 review requests。 |
| `scaffold_frontier_research_queue_input_bindings.py` | 将 input collection requests 展开为可编辑的 source-backed binding JSONL 骨架；如果能找到 rule-input-tasks JSONL，就按 task 逐行展开；对 `source_family_url_seeds` 则把 collection tasks 展开成空 URL seed 骨架，供 official/news/domain adapter 的 `--seeds` 审查绑定。 |
| `audit_frontier_research_queue_input_bindings.py` | 在 rule-input fill 或 seed adapter 执行前审计已编辑的 source-backed sidecar，检查 review、citation/value/URL、non-evidence 标记、保留字段泄漏、numeric subject-binding 合并状态和 source-family URL seed ready 状态。 |
| `bind_frontier_research_queue_seed_inputs.py` | 将 audit-ready 的 `source-family-url-seeds.jsonl` 路径回写到 frontier command bindings，替换 `--seeds ...`，但不批准绑定、不执行 adapter，也不把 URL seed 当作 verifier evidence。 |
| `run_frontier_research_queue_input_fill_commands.py` | 消费 binding audit，只 dry-run 或显式执行 audit-ready 的 fill 命令，并检查 planned outputs 是否落盘。 |
| `rollup_frontier_research_queue_input_fill_results.py` | 聚合已物化的 fill outputs，生成单个 adapter-ready rule-input JSONL，并保留未填任务队列。 |
| `run_frontier_research_queue_rule_adapter_promotion_workflow.py` | 将 fill-result rollup 串到 rule adapter 执行、candidate promotion gate、可选 ProductTrace/action-result handoff 与可选 mechanism handoff evidence bundle；默认只生成计划，`--execute` 才运行本地 gate 并写 manifest/registry metadata。 |
...
make perf-check     # deterministic profile/cache/worker/registry/product/frontier/ProductTrace smokes; no model load
