...
| `export_product_promotion_contract.py` | 从 promoted release candidate 导出紧凑的可部署 `ProductPromotionContract` JSON，包含 verifier-route covered-fact property scope、product-trace replay action-audit/action-execution gate provenance、runtime-drift promotion/triple-audit/action-gate evidence、selfcheck-signal-fusion provenance、world-model signal release-gate provenance、triple-extraction fixture-matrix provenance 和可选反馈派生 `ControlPolicyConfig`，写入 manifest，并可登记 `product_promotion_contract:*:*` handoff artifact。 |
