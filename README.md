...
| `ProductPromotionContract` / `ProductRuntimeEvidenceBundle` | Converts promoted release-candidate reports into product runtime, verifier-route, covariance quality-gate provenance, performance-bundle, performance score-dump cache, product-trace-replay workflow provenance, selfcheck-signal-fusion sample-quality/fusion provenance, world-model signal release-gate provenance, feedback-policy workflow provenance including final-answer feedback rates, selector-replay, runtime-drift evidence, release-efficiency handoff evidence, budget-policy contracts, validated feedback-derived `ControlPolicyConfig`, and candidate control defaults consumed by the calibrated-control demo; `load_product_promotion_contract()` loads compact contracts, while `load_product_runtime_evidence_bundle()` lazily attaches optional promotion-contract manifest/registry provenance plus explicit selfcheck-signal-fusion and world-model-signal workflow manifest verification and registry metadata for runtime traces. |
...
| `export_product_promotion_contract.py` | 从 promoted release candidate 导出紧凑的可部署 `ProductPromotionContract` JSON，包含 selfcheck-signal-fusion provenance、world-model signal release-gate provenance 和可选反馈派生 `ControlPolicyConfig`，写入 manifest，并可登记 `product_promotion_contract:*:*` handoff artifact。 |
