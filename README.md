...
| `compare_route_baselines.py` | Compares registered verifier-route promotion manifests or covered-facts route-summary manifests by verified state, route quality, false support/refutation, tail latency, retrieval cost, optional retrieval provenance-filter gates, and optional answer-echo retrieval stress-control gates. |
| `compare_external_evidence_baselines.py` | Combines route-baseline promotion, answer-echo stress-control evidence, and text/length redline score-ensemble reports into one fail-closed external-evidence comparison artifact, with optional manifest verification and `report:*:*` registry handoff. |
...
| `compare_product_runtime_baselines.py` | Compares current product runtime baselines against a file or registered baseline, emitting fail-closed drift gates over latency, route cost, retrieval use, cache reuse, verifier skip rate, promotion-contract/triple-extraction evidence coverage and quality drift, trace count, and an optional reusable runtime budget policy. |
...
| `compare_release_candidates.py` | Combines registered readiness, route, optional required route-baselines including robustness evidence, performance-baseline, product-trace-replay workflow file or registry key, selfcheck-signal-fusion workflow file or registry key, world-model-signal workflow file or registry key, feedback-policy workflow file or registry key, selector-replay, product-runtime-drift, release-efficiency, external-evidence baseline comparison file or registry key, adapter-family evidence, and triple-extraction fixture matrix evidence into one fail-closed release candidate with runtime flags, verifier route, quality, runtime cost, covariance quality-drop gate, performance evidence bundle/cache gates, optional performance trend gates against an explicit prior baseline, selfcheck sample-quality/fusion gates, world-model trace-gap/conflict-calibration gates, audit-route budgets, retrieval provenance-filter gates, retrieval stress-control gates, strict-audit adapter-family profile support, optional external-prediction count/corpora/F1 gates for learned triple extractors, named release-policy profiles such as `strict_structured_fact` and `frontier_audit` where `frontier_audit` defaults `--require-product-runtime-drift-promotion-evidence`, validated feedback-policy config/control-default provenance, action-level and final-answer feedback safety metrics, runtime drift provenance including promotion evidence summaries, release-efficiency profile handoff, and latency/balanced/audit profiles. |
| `run_release_candidate_registry_workflow.py` | Runs the release-candidate gate, writes a manifest covering the candidate report plus selected readiness/route/performance/product-trace-replay/selfcheck-signal-fusion/world-model-signal/feedback-policy/selector/runtime-drift/release-efficiency manifests, optional external-evidence comparison reports supplied by file or registry key, and optional required route/adapter-family/triple-extraction fixture-matrix reports, supports named release policy profiles such as `strict_structured_fact` and `frontier_audit`, can explicitly require canonical plus paraphrase `structured_fact` robustness evidence, recursively verifies it, and registers the final candidate with runtime-profile, covariance quality gate, performance-baseline, performance evidence bundle readiness/cost/cache/trend, product-trace-replay, selfcheck signal-fusion workflow status/report/record/sample-quality metadata, world-model signal workflow gate/report/record metadata, feedback-policy workflow status/report/record/candidate-policy metadata, selector-replay, runtime-drift, release-efficiency, external-evidence comparison source/record metadata, route-budget, retrieval provenance-filter policy metadata, release-policy profile, structured-fact robustness, adapter-family metadata, cross-corpus triple-extraction evidence, and optional external-prediction gate metadata; compare, manifest-build, promotion verification, and workflow-report writes emit phase timing, compare/manifest/promotion share fingerprint and JSON artifact caches, `--fingerprint-cache` persists file fingerprints, `--artifact-json-cache` persists parsed JSON artifact payloads across repeated release checks, and `--manifest-fingerprint-workers` enables bounded parallel artifact fingerprinting for local release gates. |
...
| `ProductPromotionContract` / `ProductRuntimeEvidenceBundle` | 将已 promoted release-candidate report 转成产品 runtime、verifier route、covariance 质量 gate provenance、performance-bundle、performance score-dump cache、product-trace-replay workflow provenance、selfcheck-signal-fusion 样本质量/融合证据 provenance、world-model signal release-gate provenance、包含 final-answer 反馈率的 feedback-policy workflow provenance、selector-replay、包含 promotion-contract coverage 与 triple-extraction fixture-matrix drift 摘要的 runtime-drift evidence、release-efficiency handoff evidence、budget policy contract、已校验的反馈派生 `ControlPolicyConfig` 和可被 calibrated-control demo 消费的候选 control defaults；`load_product_promotion_contract()` 加载 compact contract，`load_product_runtime_evidence_bundle()` 延迟附加可选 promotion-contract manifest/registry provenance，并可显式验证 selfcheck-signal-fusion 与 world-model-signal workflow manifest、输出 registry metadata 到 runtime trace。 |
| `RiskController` / `ParticipationGateConfig` / `ProductTrace` / `FinalAnswer` | 将校准诊断和可选验证结果转为结构化路由决策、JSON trace 和保守最终响应 payload；可选 conformal abstention gate 会在被接受答案的 uncertainty score 落到保留参与区间外时升级到 abstain/clarify；`ControlPolicyConfig.participation_gate_supported_override` 可选择开启验证感知覆盖，只有全部 claim 都被高置信 verifier 支持时才保留原本会被 participation gate 拒答的 accepted answer；非法诊断值会路由到 `clarify/unknown`，verification plan 会记录 claim 级 route/tool 意图和成本估计，route summary 会暴露选中、匹配和跳过的 verifier 工具，runtime/cache/tail-latency/route-cost summary 支持可选预算门禁，`ProductTrace` 可携带最终响应用于 replay/feedback/runtime 审计，`ProductTrace.to_bounded_dict()` 可输出带 final-answer summary 的更小线上 telemetry payload，corpus/replay/runtime-baseline 工具继续使用完整 trace 作为复现输入，`finalize_loop_answer()` 可把验证闭环折叠成 answered/abstained/needs-retrieval/needs-clarification 状态且不隐藏调用 rewrite LLM。 |
...
| `compare_route_baselines.py` | 按 manifest 验证状态、route 质量、误支持/反证率、尾延迟、retrieval 成本、可选 retrieval provenance-filter gate 和可选 answer-echo retrieval stress-control gate 比较已注册 verifier-route promotion baseline 或 covered-facts route-summary manifest。 |
| `compare_external_evidence_baselines.py` | 将 route-baseline promotion、answer-echo stress-control 证据和 text/length redline score-ensemble report 合并成一个 fail-closed 外部证据比较 artifact，并可写入 manifest、递归校验、登记成 `report:*:*`。 |
...
| `compare_product_runtime_baselines.py` | 将当前 product runtime baseline 与文件或 registry 中的基线比较，对 latency、route cost、retrieval 使用、cache 复用、verifier skip rate、promotion-contract / triple-extraction evidence 覆盖与质量漂移、trace 数量和可选复用 runtime budget policy 输出 fail-closed drift gate。 |
...
| `compare_release_candidates.py` | 将已注册 readiness baseline、route baseline、可选 required route-baseline/robustness evidence、performance baseline、product-trace-replay workflow 文件或 registry key、selfcheck-signal-fusion workflow 文件或 registry key、world-model-signal workflow 文件或 registry key、feedback-policy workflow 文件或 registry key、selector-replay、product-runtime-drift、release-efficiency、external-evidence baseline comparison 文件或 registry key、adapter-family 证据和 triple-extraction fixture matrix 证据合成一个 fail-closed release candidate，输出 runtime flags、verifier route、质量、runtime cost、covariance 质量跌幅 gate、performance evidence bundle/cache gate、可选显式 prior baseline 性能趋势 gate、selfcheck 样本质量/融合 gate、world-model trace-gap/conflict-calibration gate、audit-route budget、retrieval provenance-filter gate、retrieval stress-control gate、strict-audit adapter-family profile、可选 learned triple extractor external-prediction 数量/语料/F1 gate、`strict_structured_fact` / `frontier_audit` 等命名 release-policy profile，其中 `frontier_audit` 默认启用 `--require-product-runtime-drift-promotion-evidence`、已校验 feedback-policy config / control defaults provenance、action 级与 final-answer 反馈安全指标、包含 promotion evidence summary 的 runtime drift provenance、release-efficiency profile handoff，以及 latency/balanced/audit runtime profiles。 |
| `run_release_candidate_registry_workflow.py` | 执行 release-candidate gate，写入覆盖 candidate report、选中 readiness/route/performance/product-trace-replay/selfcheck-signal-fusion/world-model-signal/feedback-policy/selector/runtime-drift/release-efficiency manifests、可由文件或 registry key 提供的 external-evidence comparison report 和可选 required route / adapter-family / triple-extraction fixture-matrix report 的 manifest，支持 `strict_structured_fact` / `frontier_audit` 等命名 release policy profile，可显式要求 canonical + paraphrase `structured_fact` robustness evidence，递归验证后登记带 runtime-profile、covariance 质量 gate、performance-baseline、performance evidence bundle readiness/cost/cache/trend、product-trace-replay、selfcheck signal-fusion workflow 状态/report/record/样本质量 metadata、world-model signal workflow gate/report/record metadata、feedback-policy workflow 状态/report/record/候选策略 metadata、selector-replay、runtime-drift、release-efficiency、external-evidence comparison 来源/record metadata、route-budget、retrieval provenance-filter policy metadata、release-policy profile、structured-fact robustness、adapter-family metadata、跨语料 triple-extraction evidence 和可选 external-prediction gate metadata 的最终候选；compare、manifest build、promotion verification 和 workflow report write 输出 phase timing，compare/manifest/promotion 共享 fingerprint 与 JSON artifact cache，`--fingerprint-cache` 可在重复 release check 间持久化复用 fingerprint，`--artifact-json-cache` 可持久化复用已解析 JSON artifact，`--manifest-fingerprint-workers` 可为本地 release gate 开启有界并行 artifact fingerprint。 |
...
| `export_product_promotion_contract.py` | 从 promoted release candidate 导出紧凑的可部署 `ProductPromotionContract` JSON，包含 selfcheck-signal-fusion provenance、world-model signal release-gate provenance、triple-extraction fixture-matrix provenance 和可选反馈派生 `ControlPolicyConfig`，写入 manifest，并可登记 `product_promotion_contract:*:*` handoff artifact。 |
