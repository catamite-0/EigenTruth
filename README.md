...
| `run_frontier_research_queue_rule_adapter_promotion_workflow.py` | Connects a fill-result rollup to rule-adapter execution, candidate promotion, and optional ProductTrace/action-result handoff; default mode is plan-only, while `--execute` runs local gates and writes manifest/registry metadata. |
...
| `run_frontier_research_queue_rule_adapter_promotion_workflow.py` | 将 fill-result rollup 串到 rule adapter 执行、candidate promotion gate 与可选 ProductTrace/action-result handoff；默认只生成计划，`--execute` 才运行本地 gate 并写 manifest/registry metadata。 |
