...
| `TemporaryActivationIntervention` / `apply_activation_intervention` | Provides model-side forced-answer activation ablation/patching primitives over prompt, answer, all, last-token, or first-answer spans. `eval_truthfulqa.py --activation-intervention-layer ...` can produce an intervention score dump that is comparable against a baseline dump with `eval_pathway_intervention.py`. |
...
| `TemporaryActivationIntervention` / `apply_activation_intervention` | 提供模型侧 forced-answer activation ablation/patching 原语，可作用于 prompt、answer、all、last-token 或 first-answer span；`eval_truthfulqa.py --activation-intervention-layer ...` 可生成干预 score dump，再用 `eval_pathway_intervention.py` 与 baseline dump 对比。 |
