...
| `run_source_family_structured_qa_claim_correction_workflow.py` | Runs source-family claim mapping, gap triage, and correction handoff as one manifest-backed workflow for existing covered-fact QA corpora, promoting only exact mapped candidates from promoted route summaries. |
...
| `run_source_family_structured_qa_claim_correction_workflow.py` | Runs claim mapping, gap triage, and correction handoff as a single manifest-backed source-family covered-fact workflow, leaving weak/no-fact rows in triage. |
...
| `run_source_family_structured_qa_claim_correction_workflow.py` | 将 source-family claim mapping、gap triage 和 correction handoff 串成一个带 manifest 的 covered-fact workflow；只提升来自 promoted route summary 的精确映射候选。 |
...
| `run_source_family_structured_qa_claim_correction_workflow.py` | 将 claim mapping、gap triage、correction handoff 合成单命令 source-family covered-fact workflow；弱匹配和无 fact 行仍留在 triage。 |
