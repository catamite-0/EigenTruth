...
handoff artifacts are actually present in the checkout:
...
| `audit_frontier_artifact_references.py` | Scans the active frontier/product handoff references in docs, checks whether the referenced local artifacts exist, verifies referenced artifact manifests when present, and can write a registry-backed JSON report so docs do not drift from reproducible release inputs. |
...
| `audit_frontier_artifact_references.py` | 扫描文档中的当前 frontier/product handoff artifact 引用，检查本地文件是否存在，并在 manifest 存在时执行校验，可输出 registry-backed JSON report，避免实验记录和可复现 release 输入漂移。 |
