...
| `RiskController` / `ParticipationGateConfig` / `ProductTrace` / `FinalAnswer` | Converts calibrated diagnostics plus optional verification results into structured routing decisions, JSON-ready traces, and conservative final answer payloads; optional conformal abstention gates can escalate accepted answers to abstain/clarify when a selected uncertainty score falls outside the retained participation region, `ControlPolicyConfig.participation_gate_supported_override` can opt into all-supported high-confidence verifier evidence preserving an otherwise accepted answer, invalid diagnostic values route to `clarify/unknown`, verification plans record claim-level route/tool intent, cost estimates, and optional verification-budget summaries, route summaries expose selected/matched/skipped verifier tools, claim-risk localization summaries expose risky text spans and claim ids, triple-coverage summaries expose claim triples plus recorded slot-audit coverage, runtime/cache/tail-latency/route-cost summaries support optional budget gates, `ProductTrace` can carry the final answer for replay/feedback/runtime audits, `ProductTrace.to_bounded_dict()` emits smaller online telemetry payloads with final-answer, verification-budget, and claim-risk localization summaries while corpus/replay/runtime-baseline tools keep full-trace reproduction inputs, and `finalize_loop_answer()` folds a verified control loop into answered/abstained/needs-retrieval/needs-clarification status without hidden rewrite. |
