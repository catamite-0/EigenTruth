...
| `build_triple_extraction_fixture.py` | Generates labeled triple-extraction records and default regex templates from structured fact corpora, so KG-covered facts can become reproducible extractor benchmarks. |
| `run_triple_extraction_fixture_workflow.py` | Builds generated triple-extraction fixtures, evaluates rule-based/regex/composite extractors, writes per-extractor reports plus a promotion summary and artifact manifest for release evidence. |
| `run_triple_extraction_fixture_matrix.py` | Runs generated triple-extraction fixture workflows across multiple structured-fact corpora, requiring cross-corpus promotion and predicate diversity before extractor templates are treated as release evidence. |
