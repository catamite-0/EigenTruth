...
| `eval_verifier_ensemble.py` | Benchmarks calibrated internal diagnostics combined with retrieval/verifier suppression/refutation policies, optional staged verifier gating, optional triple-evidence and fact-selfcheck route audits, route-level cost metrics, and optional JSONL sidecar records for per-claim verifier outputs. |
...
| `run_verifier_signal_fusion_workflow.py` | Runs the no-model local evidence loop end to end: retrieval/selfcheck/fact-selfcheck fixture, optional retrieval provenance filtering, verifier sidecar, verifier-signal score dumps, geometry-fusion report, deployable geometry artifacts, and manifest verification. |
...
| `FactSelfConsistencyVerifier` / `FactSelfConsistencyReport` | 在 sampled responses 上抽取 fact triples，按精确 triple 支持、同 subject/predicate 不同 object 反证、未提及则证据不足的保守规则生成 claim-level 事实自一致性报告；这是 FactSelfCheck 方向的 dependency-free 桥接，不等同于外部事实证明。 |
...
| `eval_verifier_ensemble.py` | 评估校准内部诊断与 retrieval/verifier 抑制误报、补充反证检出的组合策略，可选 staged verifier gating、triple-evidence 和 fact-selfcheck route audit，记录 route 级成本指标，并可用 JSONL sidecar 保存逐 claim verifier 输出。 |
...
| `run_verifier_signal_fusion_workflow.py` | 端到端运行无模型本地证据闭环：retrieval/selfcheck/fact-selfcheck fixture、可选 retrieval provenance filtering、verifier sidecar、verifier-signal score dump、geometry-fusion report、可部署 geometry artifact 和 manifest verification。 |
