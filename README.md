...
| `compare_external_evidence_baselines.py` | Combines route-baseline promotion, answer-echo stress-control evidence, text/length redline score-ensemble reports, and optional covered-facts structured route gates into one fail-closed external-evidence comparison artifact, with optional manifest verification and `report:*:*` registry handoff. |
