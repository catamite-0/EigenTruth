...
| `summarize_unresolved_frontier_evidence.py` | 将 unresolved blind-spot queue、citation/source-family workflow、source-family coverage audit、world-model rule-input/promotion gate、rule-stub requeue report、requeued input plan 和 mechanism handoff bundle 聚合成只读 closure summary，输出 lane status、next actions、manifest/registry metadata；world-model lane 会扣除已 promoted 的 rule-family，可接入可选 rule-input audit/requeue artifact，并同时输出 raw numeric/temporal 缺口、audit-adjusted entity/numeric/temporal required inputs、已 requeue 数和 outstanding requeue 数，避免已闭合的 mechanism 候选或已重排任务污染下一轮填充计划；它不生成 verifier evidence，也不关闭 release gate。 |
| `plan_frontier_research_queue_commands.py` | 将刷新后的 frontier status report、evidence-gap plan 或 unresolved frontier evidence summary 转成不执行命令的 command plan，记录 action provenance、queue lifecycle/source-alignment metadata、命令模板、placeholder 数、required inputs、planned outputs，支持显式 `--only-active-research-queue` 过滤 closed/superseded status queue，并可写 manifest/registry metadata；unresolved summary 会把 blocked citation alignment 和 partial/audit-requeued world-model rule lane 降成可审查命令模板，但不会填入证据值。 |
