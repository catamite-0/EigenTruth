...
| `run_frontier_research_queue_rule_adapter_promotion_workflow.py` | Dry-runs or explicitly executes the deterministic rule-authoring adapter plus rule-candidate promotion gate from a fill-result rollup, fail-closing when the rollup, rule stubs, or combined rule-input sidecar are not adapter-ready. |
...
| `run_frontier_research_queue_rule_adapter_promotion_workflow.py` | Connects a fill-result rollup to rule-adapter execution and candidate promotion; default mode is plan-only, while `--execute` runs both local gates and writes manifest/registry metadata. |
...
| `run_frontier_research_queue_rule_adapter_promotion_workflow.py` | 将 fill-result rollup 串到 rule adapter 执行与 candidate promotion gate；默认只生成计划，`--execute` 才运行两个本地 gate 并写 manifest/registry metadata。 |
