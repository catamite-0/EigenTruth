...
| `EvidenceAcquisitionConformalCalibrator` / `EvidenceAcquisitionCalibrationReport` / `audit_evidence_acquisition_risk` / `audit_evidence_acquisition_anytime_risk` / `evidence_acquisition_record_from_trace` | Calibrates the whole post-acquisition answer/acquire/abstain policy score, compares a naive pre-acquisition threshold against the post-policy threshold, saves the post-policy threshold as a regular `CalibrationArtifact`, can extract labeled calibration rows from saved `ProductTrace` payloads or trace+feedback joins, alpha-spends finite feedback-prefix checks, and optionally runs a dependency-free anytime-valid betting/e-value monitor over accepted-error feedback to flag deployed acquisition drift without recalibrating on the same stream. |
