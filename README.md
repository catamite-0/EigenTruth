...
| `ProductPromotionContract` / `ProductRuntimeEvidenceBundle` | 将已 promoted release-candidate report 转成产品 runtime、verifier route、covariance 质量 gate provenance、performance-bundle、performance score-dump cache、product-trace-replay workflow provenance、selector-replay、runtime-drift evidence 和 budget policy contract；`load_product_promotion_contract()` 加载 compact contract，`load_product_runtime_evidence_bundle()` 延迟附加可选 manifest verification 与 registry provenance。 |
