...
| `stage_frontier_research_queue_binding_suggestions.py` | Copies only non-review-required scaffold suggestions into partial `bound_commands` for review, leaving source-backed inputs, upstream child-output links, review status, and execution gates unapproved. |
...
| `stage_frontier_research_queue_binding_suggestions.py` | 只把 scaffold 中不需要人工审查的机械建议写入 partial `bound_commands`，保留 source-backed inputs、上游 child-output 链接、review status 和执行 gate 的未批准状态。 |
