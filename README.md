...
| `compare_product_runtime_baselines.py` | Compares current product runtime baselines against a file or registered baseline, emitting fail-closed drift gates over latency, route cost, retrieval use, cache reuse, verifier skip rate, promotion-contract/triple-extraction evidence coverage and quality drift, covered-fact per-property rollup drift, trace-level triple/slot-audit coverage, trace count, and an optional reusable runtime budget policy. |
...
| `run_product_trace_replay_workflow.py` | Runs the raw-trace handoff end to end: redacted trace corpus, runtime-pair index, product runtime baseline, optional action-audit release gate over missing retrieval actions, uncovered planned retrieval queries, and malformed/unexpected action payloads, selector replay, optional product-runtime drift/policy/promotion-evidence and covered-fact property drift gates, top-level runtime `optimization` summary, optional recommended runtime policy artifact, recursive manifest, optional manifest verification, optional manifest fingerprint cache reuse, optional whole-corpus cache reuse, optional corpus source-cache reuse, optional runtime trace-record cache reuse, optional runtime trace-scan parallelism, optional selector trace-input cache reuse, phase timing/cache summaries, and registry-ready workflow report. |
...
| `compare_product_runtime_baselines.py` | 将当前 product runtime baseline 与文件或 registry 中的基线比较，对 latency、route cost、retrieval 使用、cache 复用、verifier skip rate、promotion-contract / triple-extraction evidence 覆盖与质量漂移、covered-fact per-property rollup drift、trace 级 triple/slot-audit 覆盖、trace 数量和可选复用 runtime budget policy 输出 fail-closed drift gate。 |
