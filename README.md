...
| `compare_product_runtime_baselines.py` | Compares current product runtime baselines against a file or registered baseline, emitting fail-closed drift gates over latency, route cost, retrieval use, cache reuse, verifier skip rate, promotion-contract/triple-extraction evidence coverage and quality drift, covered-fact per-property rollup drift, product-trace action-audit/action-execution gate rate drift, trace-level triple/slot-audit coverage, trace count, and an optional reusable runtime budget policy. |
...
| `compare_product_runtime_baselines.py` | 将当前 product runtime baseline 与文件或 registry 中的基线比较，对 latency、route cost、retrieval 使用、cache 复用、verifier skip rate、promotion-contract / triple-extraction evidence 覆盖与质量漂移、covered-fact per-property rollup drift、product-trace action-audit / action-execution gate rate drift、trace 级 triple/slot-audit 覆盖、trace 数量和可选复用 runtime budget policy 输出 fail-closed drift gate。 |
