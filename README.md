...
| `uncertainty_escalation_report` | Summarizes `run_verification_loop()` results with opt-in uncertainty escalation, including trigger rate, route/retrieval usage, decision changes, and optional label-conditioned false-accept/selective-accuracy deltas. |
...
| `eval_score_ensemble.py` | Benchmarks direction-aware rank fusion plus geometry-by-uncertainty interaction and GLU/product fusion across saved diagnostic score dumps, and can save deployable `RankScoreFusionArtifact` or `GeometryScoreFusionArtifact` outputs in one run. |
...
| `build_citation_search_adapter_handoff.py` | Converts unresolved citation/search queue items into sanitized external-adapter requests, including claim/entity-aware alternate queries, and can ingest returned search-result JSONL into an external retrieval corpus without copying labels, record ids, target ids, or model answers. |
| `run_citation_search_evidence_workflow.py` | Runs returned citation/search adapter JSONL through handoff ingestion, external-corpus provenance audit, blind-spot query sweep, configurable target-route gating, and optional controlled-vs-external comparison before any route promotion. |
...
| `run_official_site_source_family_catalog_adapter.py` | Fetches URL-seeded official pages into an adapter-ready `source_family=official` catalog, preserving URL/task provenance while keeping request ids, labels, and model answers out of source documents. |
...
| `rollup_frontier_stability_evidence_reruns.py` | Reads a frontier stability rerun queue plus completed verifier/abstention stability reports, reapplies release thresholds, and writes manifest/registry-ready stability evidence. |
| `plan_frontier_abstention_evidence_reruns.py` | Turns blocked frontier abstention participation gates into a profile x signal-family `eval_abstention_stability.py` experiment queue, including supervised-feasibility refresh commands and registry/manifest provenance. |
| `rollup_frontier_abstention_evidence_reruns.py` | Reads a frontier abstention rerun queue plus completed `eval_abstention_stability.py` reports, selects the best promotion-eligible participation-gate candidate, and writes manifest/registry-ready release evidence. |
| `rollup_frontier_detectability_evidence_reruns.py` | Reads a frontier detectability rerun queue plus completed taxonomy or blind-spot reports, promotes passing taxonomy reruns, and records completed blind-spot audits without treating diagnostics as release fixes. |
| `rollup_frontier_multiple_testing_reruns.py` | Reads a frontier multiple-testing rerun queue plus completed single-cell `run_truthfulqa_frontier_workflow.py` reports, verifies both the top-level family-wise gate and queued cell artifact paths, and writes manifest/registry-ready release evidence. |
...
| `compare_product_runtime_baselines.py` | Compares current product runtime baselines against a file or registered baseline, emitting fail-closed drift gates over latency, route cost, retrieval use, cache reuse, verifier skip rate, trace-level world-model participation/coverage/conflict/low-agreement/trace-gap drift, promotion-contract/triple-extraction evidence coverage and quality drift, pre-generation probe comparison/redline handoff drift, counterfactual verifier-audit handoff drift, frontier release-evidence citation/source-family batch track drift, promotion-contract evidence-handoff coverage/manifest/metric-gap/group-status drift, covered-fact per-property rollup drift, product-trace action-audit/action-execution gate rate drift, trajectory-audit factual/referential/logical/procedural/scope drift, trace-level triple/slot-audit coverage, trace count, and an optional reusable runtime budget policy. |
...
| `run_source_family_structured_qa_fact_collection_workflow.py` | Runs the fact-collection sidecars through local source-family catalogs, writes candidate adapter results, rebuilds a structured QA candidate corpus, and keeps rule-authoring stubs as non-evidence inputs. |
...
| `uncertainty_escalation_report` | 汇总带 uncertainty escalation 的 `run_verification_loop()` 结果，输出触发率、route/retrieval 使用率、决策迁移，以及可选标签条件下的 false-accept 和 selective-accuracy 变化。 |
...
| `eval_score_ensemble.py` | 对已保存诊断 score dump 执行方向感知 rank fusion、geometry-by-uncertainty interaction fusion 和 GLU/product fusion benchmark，并可在一次运行中保存可部署的 `RankScoreFusionArtifact` 或 `GeometryScoreFusionArtifact`。 |
...
| `build_citation_search_adapter_handoff.py` | 将 unresolved citation/search queue 转成去敏的外部 adapter 请求，支持 claim/entity-aware alternate queries，并可把外部搜索结果 JSONL 回填为 external retrieval corpus；不会复制 label、record id、target id 或 model answer。 |
| `run_citation_search_evidence_workflow.py` | 将 citation/search adapter 回填 JSONL 串过 handoff ingestion、external-corpus provenance audit、可配置目标路由的 blind-spot query sweep 和可选 controlled-vs-external comparison，再决定能否进入 route promotion。 |
...
| `run_official_site_source_family_catalog_adapter.py` | 将 URL-seeded 官方网页抓取成 adapter-ready `source_family=official` catalog，保留 URL/task provenance，但不把 request id、label 或 model answer 写进 source docs。 |
...
| `run_source_family_structured_qa_fact_collection_workflow.py` | 用本地 source-family catalog 执行这些 fact-collection 请求桶，产出候选 adapter matches，重建 structured QA candidate corpus，并把 world-model/calculator rule stub 保持为非证据输入；post-correction replay 复用同一边界后再跑 route 与 claim-mapping gate。 |
...
| `rollup_frontier_stability_evidence_reruns.py` | 汇总 frontier stability rerun queue 和完成的 verifier/abstention stability 子报告，重新应用 release 阈值，并输出可写 manifest/registry 的 stability evidence。 |
| `rollup_frontier_multiple_testing_reruns.py` | 汇总 frontier multiple-testing rerun queue 和单 cell `run_truthfulqa_frontier_workflow.py` 子报告，同时检查顶层 family-wise gate 与队列 cell artifact 路径，输出可写 manifest/registry 的 release evidence。 |
...
| `compare_product_runtime_baselines.py` | 将当前 product runtime baseline 与文件或 registry 中的基线比较，对 latency、route cost、retrieval 使用、cache 复用、verifier skip rate、promotion-contract / triple-extraction evidence 覆盖与质量漂移、pre-generation probe comparison / redline handoff 漂移、counterfactual verifier-audit handoff 漂移、frontier release-evidence citation/source-family batch track 漂移、promotion-contract evidence-handoff 覆盖/manifest/metric-gap/group-status 漂移、covered-fact per-property rollup drift、product-trace action-audit / action-execution gate rate drift、trajectory-audit factual/referential/logical/procedural/scope drift、trace 级 triple/slot-audit 覆盖、trace 数量和可选复用 runtime budget policy 输出 fail-closed drift gate。 |
...
| `run_source_family_structured_qa_fact_collection_workflow.py` | 将 fact-collection sidecar 接到本地 source-family catalog，写出候选 adapter results，重建 structured QA candidate corpus，并保留 rule-authoring stub。 |
