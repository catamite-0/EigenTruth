...
| `EvidenceAcquisitionConformalCalibrator` / `EvidenceAcquisitionCalibrationReport` | Calibrates the whole post-acquisition answer/acquire/abstain policy score, compares a naive pre-acquisition threshold against the post-policy threshold, and saves the post-policy threshold as a regular `CalibrationArtifact` for reproducible evidence-acquisition experiments. |
...
| `EvidenceAcquisitionConformalCalibrator` / `EvidenceAcquisitionCalibrationReport` | 对完整的 post-acquisition answer/acquire/abstain policy score 做 conformal 校准，比较 naive pre-acquisition threshold 与 post-policy threshold，并把后者保存成普通 `CalibrationArtifact`；这是预算化证据获取 trace 与有限样本选择性回答保证之间的可复现实验桥。 |
