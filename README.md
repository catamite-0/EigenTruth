...
| `summarize_unresolved_frontier_evidence.py` | 将 unresolved blind-spot queue、citation/source-family workflow、source-family coverage audit、semantic-gap review workflow、world-model rule-input/promotion gate、rule-stub requeue report、requeued input plan 和 mechanism handoff bundle 聚合成只读 closure summary，输出 lane status、next actions、manifest/registry metadata；citation lane 会保留 blocked query-sweep 的失败原因和推荐动作计数，semantic-gap lane 可消费 reviewed covered-fact route 结果，world-model lane 会扣除已 promoted 的 rule-family 与已重排任务，避免已闭合候选污染下一轮计划；frontier queue lane 还可记录 `frontier_queue_execution_smoke.py` 的控制面健康状态和递归 manifest 校验，但不会把 smoke 当作闭合证据；它不生成 verifier evidence，也不关闭 release gate。 |
| `plan_frontier_research_queue_commands.py` | 将刷新后的 frontier status report、evidence-gap plan、source-family catalog collection plan、source-family structured-QA lane rerun queue 或 unresolved frontier evidence summary 转成不执行命令的 command plan，记录 action provenance、queue lifecycle/source-alignment metadata、命令模板、placeholder 数、required inputs、planned outputs，支持显式 `--only-active-research-queue` 过滤 closed/superseded status queue，并可写 manifest/registry metadata；unresolved summary 会把 semantic-gap covered-fact review、blocked citation alignment、partial/audit-requeued world-model rule lane，以及 frontier queue review/re-bind/execute/repair lane 降成可审查命令模板，但不会填入证据值；citation alignment 模板会根据 query-sweep 诊断切换 query mode、target route、retrieval/verifier 阈值，并补 source-family coverage audit/collection 后续命令；source-family collection plan 会按 ready provider family 生成 Crossref、OpenAlex、World Bank、GDELT、seeded URL 或 official-site catalog adapter 模板；structured-QA lane rerun queue 则把 lane batch 命令接入同一套 binding/review/run 控制面。 |
