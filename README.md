...
| `run_product_runtime_baseline.py` | 聚合已保存的 `ProductTrace` JSON，输出请求级 runtime baseline：phase、cache、verifier route、route-budget exhaustion、retrieval 使用率、staged-verification 节省量、verification-plan 覆盖率/route/tool-payload 计数、final-answer 覆盖率/status/action 汇总、promotion-contract covered-fact property scope、runtime-profile context、带 phase/route 热点和 cache/retrieval/staging/profile 建议的 `optimization` 块、`max_verifier_route_attempts` 等候选 control-default hints、可选 `ProductRuntimeBudgetPolicy` gate、可复用推荐 policy artifact、逐 trace record 的可选 JSONL sidecar 存储、重复 budget sweep 可复用的 trace-record cache，以及大 trace 集可选并行扫描。 |
