...
| `enrich_product_trace_triple_audit.py` | 离线补强 full `ProductTrace` JSON：使用 rule-based `claim_triples`、trace evidence 和本地 JSON/JSONL evidence corpus 生成严格 triple-evidence `audit_report` metadata，写出 enriched traces、manifest-backed report 和可选 registry metadata，让 runtime baseline 能读取 trace-level triple-audit coverage，且不新增依赖。 |
