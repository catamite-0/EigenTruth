...
| `ClaimRiskSpan` / `ClaimRiskLocalizationReport` / `localize_claim_risk_spans` | Converts existing claim spans, verification results, route hints, entity/surface candidates, and verification-budget drops into dependency-free span/claim risk annotations with risk level, score, status, confidence, routes, evidence count, feature flags, entity counts, and reasons; `ProductTrace` and runtime metrics expose compact summaries for product UI and audit without requiring a learned token detector. |
...
| `ClaimRiskSpan` / `ClaimRiskLocalizationReport` / `localize_claim_risk_spans` | 将已有 claim span、verification result、route hint、实体/表面候选和 verification-budget drop 转成无依赖的 span/claim 风险标注，包含 risk level、score、status、confidence、routes、evidence count、feature flags、entity counts 和 reasons；`ProductTrace` 与 runtime metrics 会输出 compact summary，供产品 UI 和审计使用，不要求 learned token detector。 |
