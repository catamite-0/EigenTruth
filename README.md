...
| `eval_verifier_ensemble.py` | Benchmarks calibrated internal diagnostics combined with retrieval/verifier suppression/refutation policies and route-level cost metrics. |
...
| `run_local_retrieval_route_workflow.py` | Builds local retrieval evidence from score dumps and corpora, promotes the retrieval-groundedness route, fingerprints all source artifacts, records a runtime profile, optionally uses SQLite FTS/cached claims fixtures, and optionally registers the route baseline. |
...
| `eval_verifier_ensemble.py` | 评估校准内部诊断与 retrieval/verifier 抑制误报、补充反证检出的组合策略，并记录 route 级成本指标。 |
...
| `run_local_retrieval_route_workflow.py` | 从 score dump 和本地 corpus 构建 retrieval evidence，promote retrieval-groundedness route，指纹化全部源 artifact，记录运行 profile，可选使用 SQLite FTS/claims fixture 缓存，并可选注册 route baseline。 |
