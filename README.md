...
| `run_uncertainty_escalation_workflow.py` | Runs local claim/evidence fixtures through `run_verification_loop(..., escalation_policy=...)`, writes a `VerificationLoopResult` JSONL sidecar, and emits the matching escalation report without loading a model. |
...
| `run_uncertainty_escalation_workflow.py` | 将本地 claim/evidence fixture 跑过 `run_verification_loop(..., escalation_policy=...)`，写出 `VerificationLoopResult` JSONL sidecar，并直接生成对应 escalation report；全程不加载模型。 |
