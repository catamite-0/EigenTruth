...
| `run_adapter_readiness_workflow.py` | 将 adapter-family 质量门槛和 cache-profile 性能门槛合并为一个最终 adapter readiness decision，并输出可注册 manifest。 |
...
```bash
make check
make perf-check     # deterministic profile/cache-gate smokes; no model load
