...
| `scaffold_frontier_research_queue_bindings.py` | Generates a review scaffold and unbound bindings skeleton from a frontier research queue command plan, including ordered placeholder records and suggested output paths without auto-binding values. |
...
| `scaffold_frontier_research_queue_bindings.py` | 从 frontier research queue command plan 生成审查用 scaffold 和未绑定 bindings skeleton，列出有序 placeholder 与建议 output path，但不会自动绑定值。 |
