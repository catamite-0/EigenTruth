...
| `plan_frontier_research_queue_commands.py` | Converts a refreshed frontier status report or evidence-gap plan into a non-executing command plan with action provenance, queue lifecycle/source-alignment metadata, command templates, placeholder counts, required inputs, planned outputs, optional `--only-active-research-queue` filtering for closed/superseded status queues, and optional manifest/registry metadata. |
...
| `plan_frontier_research_queue_commands.py` | 将刷新后的 frontier status report 或 evidence-gap plan 转成不执行命令的 command plan，记录 action provenance、queue lifecycle/source-alignment metadata、命令模板、placeholder 数、required inputs、planned outputs，支持显式 `--only-active-research-queue` 过滤 closed/superseded status queue，并可写 manifest/registry metadata。 |
