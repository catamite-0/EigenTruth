...
| `eval_triple_extraction.py` | Evaluates dependency-free triple extractors against labeled triples, comparing rule-based, regex, regex-with-rule-based fallback, and composite extractor variants before they are promoted into structured-fact or triple-evidence routes. |
| `build_triple_extraction_fixture.py` | Generates labeled triple-extraction records and default regex templates from structured fact corpora, with optional negated near-miss, predicate-confusion, and non-assertive negative controls for KG-covered extractor benchmarks. |
| `run_triple_extraction_fixture_workflow.py` | Builds generated triple-extraction fixtures, evaluates rule-based/regex/composite extractors, writes per-extractor reports plus subgroup gates and an artifact manifest for release evidence. |
| `run_triple_extraction_fixture_matrix.py` | Runs generated triple-extraction fixture workflows across multiple structured-fact corpora, requiring cross-corpus promotion, predicate diversity, and optional adversarial subgroup gates before extractor templates are treated as release evidence. |
...
| `ClaimTriple` / `RuleBasedTripleExtractor` / `RegexTripleExtractor` / `CompositeTripleExtractor` / `TripleEvidenceVerifier` | 增加无依赖 subject-predicate-object 审计路径：简单规则、可配置 regex 模板、组合 extractor 或调用方 metadata 生成 triple，再用可通过 source、metadata、claim id 或 subject anchor 绑定的证据片段逐槽检查 subject、predicate 和 object 覆盖，然后才输出更严格的 supported 结果。 |
