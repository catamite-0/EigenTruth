...
| `internal_eigenscore` / `spectral_effective_rank` | Computes INSIDE/EigenScore-style spectral diversity from hidden-state embeddings. |
...
| `internal_eigenscore` / `spectral_effective_rank` | 基于隐藏态嵌入计算 INSIDE/EigenScore 风格的谱分散度。 |
