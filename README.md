...
| `CachedVerifier` / `CachedRetriever` / `CachedStateSource` | 为重复 verifier、retrieval 和 state-source 调用提供 request-scoped 内存缓存与 hit/miss 统计；`CachedVerifier(cache_key_mode="semantic")` 会规范化 claim 文本并忽略抽取位置噪声，同时保留 verifier 相关 metadata。 |
