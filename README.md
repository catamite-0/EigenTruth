...
| `RepresentationTelemetryRecorder` / `representation_telemetry_snapshot` | Records training-side per-layer representation telemetry without Trainer dependencies: mean norm, variance trace, spectrum rank diagnostics, and Gaussian 2-Wasserstein/Bures distance to an initialization baseline. |
...
| `RepresentationTelemetryRecorder` / `representation_telemetry_snapshot` | 无需绑定 Trainer 的训练侧逐层表征 telemetry：记录 mean norm、variance trace、谱 rank 诊断，以及到初始化 baseline 的 Gaussian 2-Wasserstein/Bures 距离。 |
