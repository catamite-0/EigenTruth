...
| `RetrievalActionExecutor` / `InMemoryRetriever` / `SQLiteFTSRetriever` / `HTTPJSONRetriever` | 为 unsupported claim 的取证流程提供无依赖 retrieval executor shell，覆盖本地片段、SQLite FTS 和调用方提供的 HTTP JSON search service；retrieval adapter 异常会 fail closed 成结构化 `ActionResult`。 |
