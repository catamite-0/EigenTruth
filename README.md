...
| `ClaimTriple` / `RuleBasedTripleExtractor` / `TripleEvidenceVerifier` | 增加无依赖 subject-predicate-object 审计路径：简单规则或调用方 metadata 生成 triple，再跨一个或多个证据片段逐槽检查 subject、predicate 和 object 覆盖，然后才输出更严格的 supported 结果。 |
