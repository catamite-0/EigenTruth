...
| `frontier_queue_execution_smoke.py` | Builds a no-model frontier queue execution fixture that writes command-plan/scaffold/staged-bindings/review/dry-run artifacts, verifies a manifest, and registers the smoke report without executing child frontier workflows. |
...
| `frontier_queue_execution_smoke.py` | 生成无需模型的 frontier queue execution fixture，落盘 command-plan/scaffold/staged-bindings/review/dry-run artifacts，验证 manifest 并登记 smoke report，但不执行子 frontier workflow。 |
