...
| `fetch_wikidata_reference_docs.py` | Materializes small CC0 Wikidata SPARQL result sets as JSONL source documents for external retrieval corpus ingestion, including country capitals and template-ready country core facts. |
...
| `fetch_wikidata_reference_docs.py` | Fetches or replays Wikidata country-capital or country-core-fact SPARQL results into JSONL source docs for external evidence smoke gates. |
...
| `fetch_wikidata_reference_docs.py` | 将小型 Wikidata CC0 SPARQL 结果物化为 JSONL source docs，用于外部 retrieval corpus ingestion，支持 country capitals 和模板可消费的 country core facts。 |
...
| `fetch_wikidata_reference_docs.py` | 拉取或重放 Wikidata country-capital / country-core-fact SPARQL 结果，输出外部证据 smoke gate 可用的 JSONL source docs。 |
