...
| `summarize_citation_binding_audit_failures.py` | Turns claim-specific citation binding audit rejects into a read-only evidence-quality review with issue buckets, question-type counts, examples, and next-action recommendations such as collecting numeric/statistical evidence, causal/procedural evidence, timestamped sources, or claim-specific evidence spans; citation evidence workflows write this review automatically when `--audit-source-bindings` is enabled. |
...
| `summarize_citation_binding_audit_failures.py` | 将 claim-specific citation binding audit 的拒绝记录整理成只读 evidence-quality review，包含 issue bucket、question type 计数、示例和下一步建议，例如采集 numeric/statistical evidence、causal/procedural evidence、timestamped source 或 claim-specific evidence span；启用 `--audit-source-bindings` 时 citation evidence workflow 会自动写出该 review。 |
