...
- **Cost:** medium.
  **Status:** `TrajectoryMonitor` and `trajectory_convergence_metrics()` landed
  ...
  layer. Larger samples and at least one additional model family are still
  required before treating trajectory scores as a calibrated hallucination
  detector.
