...
- **Cost:** low-medium. **Status:** spectrum diagnostics foundation implemented
  ...
  implemented; real AUROC comparison still pending.
...
| E2 | 2026-06-25 | partial: spectrum diagnostics and OAS-style shrinkage mode landed; real AUROC gate pending | `TruthManifold.spectrum()` / `covariance_spectrum()` / `covariance_shrinkage_intensity()` unit tests; `eval_truthfulqa.py --include-layer-spectra` tests |
| E3 | | pending | |
| E4 | | pending | |
| E5 | | pending | |
| E6 | | pending | |
