...
| E2-triple-extraction-adversarial | 2026-06-26 | **ACCEPT negated near-miss gate, KEEP covered-template scope**: `build_triple_extraction_fixture.py --adversarial-negatives-per-fact 1` adds negated near-miss records with no expected triples, and the workflow/matrix reports `best_adversarial_false_positive_rate`. The first run correctly blocked because the rule-based fallback extracted `not ...` strings as positive triples (`best_f1=0.889`, false-positive rate `1.000`). After adding explicit negation rejection to `RuleBasedTripleExtractor`, the same Wikidata country-core plus organization/product adversarial matrix promotes: country-core adds `359` negative controls, organization/product adds `8`, both best extractor F1 values are `1.000`, and `mean_best_adversarial_false_positive_rate=max_best_adversarial_false_positive_rate=0.000`. This promotes negated near-miss robustness for covered KG templates; broader predicate-confusion and non-template negatives remain separate gates. | `src/eigentruth/verify/triples.py`; `benchmarks/build_triple_extraction_fixture.py`; `benchmarks/eval_triple_extraction.py`; `benchmarks/run_triple_extraction_fixture_workflow.py`; `benchmarks/run_triple_extraction_fixture_matrix.py`; `tests/test_frontier_toolkit.py::test_rule_based_claim_triples_reject_explicit_negation`; `tests/test_benchmarks.py::test_build_triple_extraction_fixture_can_include_adversarial_negatives`; `tests/test_benchmarks.py::test_triple_extraction_fixture_workflow_promotes_when_adversarial_negatives_are_rejected`; `tests/test_benchmarks.py::test_triple_extraction_fixture_matrix_reports_adversarial_gate`; `artifacts/wikidata-cross-corpus-triple-extraction-adversarial-matrix/triple-extraction-fixture-matrix.json`; `artifacts/wikidata-cross-corpus-triple-extraction-adversarial-matrix/artifact-manifest.json`; `artifacts/wikidata-cross-corpus-triple-extraction-adversarial-matrix/manifest-verification.json` |
