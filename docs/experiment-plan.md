...
| E2-triple-extraction-adversarial | 2026-06-26 | **BLOCK adversarial extractor robustness, KEEP fail-closed gate**: `build_triple_extraction_fixture.py --adversarial-negatives-per-fact 1` now adds negated near-miss records with no expected triples, and the workflow/matrix reports `best_adversarial_false_positive_rate`. On the same Wikidata country-core plus organization/product corpora, the adversarial matrix blocks: country-core adds `359` negative controls, organization/product adds `8`, both best extractor F1 values fall to `0.889`, and `mean_best_adversarial_false_positive_rate=max_best_adversarial_false_positive_rate=1.000`. This proves the current extractor templates are not negation-robust and should not be used to claim open-domain claim extraction robustness until a stronger extractor or explicit negation handling passes this gate. | `benchmarks/build_triple_extraction_fixture.py`; `benchmarks/eval_triple_extraction.py`; `benchmarks/run_triple_extraction_fixture_workflow.py`; `benchmarks/run_triple_extraction_fixture_matrix.py`; `tests/test_benchmarks.py::test_build_triple_extraction_fixture_can_include_adversarial_negatives`; `tests/test_benchmarks.py::test_triple_extraction_fixture_workflow_blocks_adversarial_false_positives`; `tests/test_benchmarks.py::test_triple_extraction_fixture_matrix_reports_adversarial_gate`; `artifacts/wikidata-cross-corpus-triple-extraction-adversarial-matrix/triple-extraction-fixture-matrix.json`; `artifacts/wikidata-cross-corpus-triple-extraction-adversarial-matrix/artifact-manifest.json`; `artifacts/wikidata-cross-corpus-triple-extraction-adversarial-matrix/manifest-verification.json` |
