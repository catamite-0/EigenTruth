...
- **Cost:** medium.
  **Status:** `TrajectoryMonitor` and `trajectory_convergence_metrics()` landed
  ...
  accepts the reproducible real-model harness, but larger samples and at least
  one additional model family are still required before treating trajectory
  scores as a calibrated hallucination detector.
...
| E7-real-gpt2 | 2026-06-26 | **ACCEPT real-model harness, KEEP detector status preliminary**: `eval_trajectory_truthfulqa.py` now runs on real Hugging Face causal LMs. On the Qwen l80 statement-bearing score dump subset with `gpt2 --limit 64`, final layer `-1` evaluates 60 rows and reaches trajectory AUROC 0.603 with lower convergence indicating false labels; mid layer `-6` reaches AUROC 0.577 with higher convergence indicating false labels. This beats the same-subset NLL higher-is-false baseline AUROC 0.352, but the margin is modest and the sample is too small for calibration or product routing. | `benchmarks/eval_trajectory_truthfulqa.py`; `artifacts/e7-truthfulqa-gpt2-trajectory/gpt2-limit64-layer-comparison-summary.json`; `artifacts/e7-truthfulqa-gpt2-trajectory/gpt2-qwen-l80-limit64-layer-1-trajectory-report.json`; `artifacts/e7-truthfulqa-gpt2-trajectory/gpt2-qwen-l80-limit64-layer-6-trajectory-report.json` |
