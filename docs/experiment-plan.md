...
- **Cost:** medium.
  **Status:** `TrajectoryMonitor` and `trajectory_convergence_metrics()` landed
  ...
  gate: the current single-model gpt2 limit-64 artifact is explicitly blocked
  because it has only one model and 60 evaluated examples. Larger samples and
  at least one additional model family are still required before treating
  trajectory scores as a calibrated hallucination detector.
