...
- `run_inside_trigger_budget_sweep.py` wraps that profile runner across several `--inside-trigger-*` budgets, writing one sweep report with generated samples, `inside_generation`, ratios to an optional full-sample reference, inside-score AUROCs, and a lowest-cost budget recommendation.
