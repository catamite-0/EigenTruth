...
- `RuntimeProfile` now exposes shared `latency`, `balanced`, and `audit` defaults for release gates and product control-plane staging; `select_runtime_profile()` chooses among them from cheap diagnostics and claim metadata before verifier work, and `calibrated_control_demo.py --runtime-profile auto` writes the selected profile, selection reason, and staging behavior into trace and registry metadata.
