...
- `run_product_runtime_profile_sweep.py` runs deterministic calibrated-control demo scenarios across `latency`, `balanced`, and `audit` runtime profiles, writes trace artifacts, builds one ProductTrace runtime baseline per profile, ranks non-blocked profiles by request runtime and route cost, and can register the sweep report. It supports `--max-workers` profile-level concurrency while keeping trace order deterministic inside each profile; use the default single-worker mode for promotion timing evidence and multi-worker mode for faster smoke/coverage scans. It also supports `--compact-json` across generated traces, baselines, reports, and manifests without changing payload schemas. It is the lightweight local path for deciding which staged-verification profile should become the default product posture before collecting real traffic traces. Skipped verifier stages with no selected route now count as zero route cost for route-budget checks, while missing runtime traces still fail closed when runtime phase/total thresholds are configured. The current registered sweep is `report:smollm2-product-runtime-profile-sweep:0.1`, which promotes all three profiles against the strict 1.4 product runtime budget and recommends `audit` on the deterministic scenario set.
