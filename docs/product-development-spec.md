...
- `recommend_runtime_config.py` turns promoted cache-profile matrix, optional worker-sweep reports, optional INSIDE sampling profile reports, and optional trigger-budget sweep reports into one deployable runtime recommendation with layer, batch size, capture mode, covariance mode/rank, token budget, prefix-KV mode, worker count, sampling configuration, selected trigger budget, best available AUROC quality signal, covariance tradeoff details, cache-tuning advice, and equivalent benchmark flags. It emits cache-tuning recommendations when eval-reps shard hit rate is low, cross-shard reads are high, or each cache read returns too few records; when `eval_reps_shard_read_cache_sizes` has already compared several capacities, it records the swept sizes and selected read-cache size instead of emitting contradictory read-cache heuristic advice. It supports explicit trigger-budget selection policies: `quality_balanced` preserves the default release posture, `cost_first` minimizes sampled INSIDE work for latency-constrained deployments, and `quality_first` chooses the highest measured INSIDE quality metric. It preserves `--derive-from-max-budget` in generated sweep flags, performs no model work, and should be treated as the final handoff from same-machine performance evidence to local deployment defaults.
