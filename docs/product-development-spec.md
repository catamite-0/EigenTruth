...
- `ProductRuntimeBudgetPolicy` / `evaluate_product_runtime_budget` turns `ProductTrace.runtime_trace` into an optional fail-closed product runtime gate over total request time and named phase timings; `calibrated_control_demo.py --max-runtime-total-seconds/--max-runtime-phase-seconds` writes the evaluated result into trace and registry metadata.
