...
- `ScoreDump` / `load_score_dump` / `score_dump_file_metadata` provide one validated post-hoc score-dump entry point for conformal calibration and score-ensemble reports, including record counts, score names, sweep-layer metadata, file size, and SHA-256 provenance so repeated analyses can verify exact input reuse without model reruns.
