...
- `run_runtime_profile_selector_replay.py` replays multiple `RuntimeProfileSelectorPolicy` candidates over saved `ProductTrace` JSON without rerunning demos, verifiers, retrieval, or model work. It uses each trace's `risk_decision` and claim metadata to estimate selected-profile distribution, switches from the original runtime profile, configurable profile-cost units, paired observed runtime when the same logical request is available under multiple profiles, optional `--runtime-pair-index` reuse from a trace corpus, optional replay gates, an artifact manifest, and an optional `report:*:*` registry record. This is the cheaper path for validating selector changes against captured product traces once demo sweeps have established the baseline policy shape.
