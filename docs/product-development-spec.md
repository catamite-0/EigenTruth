...
- `run_adapter_readiness_workflow.py` composes adapter-family quality promotion with same-machine cache-profile performance promotion into one final fail-closed adapter readiness decision, writes `runtime-recommendation.json` from the promoted performance matrix plus optional INSIDE sampling profile evidence, and includes it in the top-level artifact manifest for recursive verification and registry promotion.
...
- `compare_readiness_baselines.py` compares registered adapter-readiness manifests without rerunning model work: it recursively verifies each manifest, regenerates runtime recommendations from saved performance matrices and optional INSIDE sampling profile artifacts when possible, applies optional quality/runtime/sampling-cost gates, uses uncached total time as a conservative forward-cost fallback for legacy reports, and recommends the passing baseline with the best AUROC quality signal.
- `compare_release_candidates.py` combines registered readiness and verifier-route baselines into the final local release gate: both underlying baseline comparisons must promote, including any readiness sampling-cost gates and route runtime/cache budget gates, then the report emits one release candidate with runtime flags, verifier route, quality evidence, runtime cost, and route cost.
- `run_release_candidate_registry_workflow.py` registers a final local release candidate by writing the release-candidate report, building a manifest over that report plus the selected readiness and route manifests, recursively verifying the whole chain, and recording the verified candidate in `ArtifactRegistry` with recommended runtime, INSIDE sampling cost evidence when available, route quality, tail latency, attempted-route, and retrieval-use metadata.
- Local release-candidate smoke artifacts: `benchmark_manifest:tiny-local-staged-qa-release-candidate:0.4` combines the tiny-gpt2 offline readiness/runtime baseline with the TruthfulQA l80 staged structured QA route baseline, and `benchmark_manifest:tiny-local-inside-staged-qa-release-candidate:0.5` adds a registered INSIDE sampling profile gate. The 0.5 smoke selects `adaptive_selfcheck` with generated-sample ratio `0.667` and `inside_generation` ratio `0.716` versus fixed sampling. These artifacts prove the registry/release-gate plumbing end to end; representative Qwen or SmolLM2 readiness artifacts are still required before treating them as model-specific deployment recommendations.
