...
- Sharded `--eval-reps-cache` readers reuse recently touched shards through a default 2-shard read-side LRU cache, reducing repeated `torch.load` calls during small-batch cache-only or cached scoring runs; `--eval-reps-shard-read-cache-size 1` restores the previous single-shard memory profile, and triplet/matrix/worker-sweep/performance-baseline profile runners pass this setting through for cached/cache-only runs. `run_cache_profile_matrix.py --eval-reps-shard-read-cache-sizes 1,2,4` promotes read-cache capacity into a gated matrix dimension with shared eval-reps cache artifacts and cell-level runtime recommendations. JSON output includes `cache_stats.eval_reps_reader` counters for read requests, records read, shard read requests, cross-shard reads, shard loads, and shard cache hits when an eval-reps reader is used, and profile summary adds `cache_efficiency.eval_reps_reader` hit-rate/read-shape metrics.
