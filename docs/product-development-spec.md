...
- Current small CPU SmolLM2 l8 performance evidence registers `performance_baseline:smollm2-l8-read-cache-worker-sweep-performance-baseline:0.1`: read-cache size `2` remains the promoted cache-only setting, worker count `2` lowers same-machine matrix wall-clock from `184.467s` to `141.385s`, and the selected cell keeps `truth_proj` AUROC `0.830`. Treat this as local CPU deployment evidence, not a cross-hardware default.
