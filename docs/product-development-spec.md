...
- Current small CPU SmolLM2 l8 performance evidence registers `performance_baseline:smollm2-l8-read-cache-worker-sweep-score-fusion-performance-baseline:0.2`: read-cache size `2` remains the promoted cache-only setting, worker count `2` lowers same-machine matrix wall-clock from `184.467s` to `141.385s`, the selected cell keeps `truth_proj` AUROC `0.830`, and the runtime recommendation carries conformal-gated `score_fusion_mean_rank` evidence from the l80 score-ensemble report (`AUROC=0.679`, false alarm `0.090`, detection `0.196`). Treat this as local CPU deployment evidence, not a cross-hardware default.
...
- `run_release_candidate_registry_workflow.py` registers a final local release candidate by writing the release-candidate report, building a manifest over that report plus the selected readiness, route, optional performance manifests, optional product-trace-replay manifests, optional selector-replay manifests, optional product-runtime-drift manifests, and optional adapter-family matrix report, recursively verifying the whole chain, and recording the verified candidate in `ArtifactRegistry` with recommended runtime, selected runtime profile, selected trigger-budget policy, covariance quality-gate metadata, INSIDE sampling / trigger-budget cost evidence when available, route quality, tail latency, attempted-route, retrieval-use metadata, performance-baseline record metadata, performance evidence bundle readiness/cost/cache/trend metadata, product-trace-replay metadata, selector replay runtime-delta metadata, product runtime drift metadata, and required adapter-family route metadata.
