...
- `production_tool_loop_demo.py` emits one product-style trace that combines SQLite pre-tool checks, side-effecting local `execute_tool`, mapped `ActionResult.output`, post-tool structured verification, action execution summary, and runtime route summary metadata.
...
- Generalize real side-effecting executors behind the same trace and tool-output mapping interfaces; the current production-like demo proves the path with a local SQLite mutation.
