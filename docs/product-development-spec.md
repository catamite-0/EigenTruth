...
- `eigentruth.control.ActionExecutionPolicy` / `PolicyGuardedActionExecutor`: validates side-effecting action requests and records audit metadata such as request ids, idempotency keys, timeout bounds, and timeout-enforcement status.
...
- `production_tool_loop_demo.py` emits one product-style trace that combines SQLite pre-tool checks, guarded side-effecting local `execute_tool`, mapped `ActionResult.output`, post-tool structured verification, action execution summary, action audit metadata, and runtime route summary metadata.
...
- Generalize real side-effecting executors behind the same trace, execution-policy, and tool-output mapping interfaces; the current production-like demo proves the path with a guarded local SQLite mutation.
