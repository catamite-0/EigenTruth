...
- `compare_readiness_baselines.py` compares registered adapter-readiness manifests without rerunning model work: it recursively verifies each manifest, regenerates runtime recommendations from saved performance matrices when possible, applies optional quality/runtime gates, and recommends the passing baseline with the best AUROC quality signal.
