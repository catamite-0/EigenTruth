...
- `run_product_runtime_baseline.py` now adds `optimization.hotspots`, `optimization.recommendations`, and `optimization.policy_hints` to product runtime baselines, so saved traces can identify dominant phases/routes, low cache hit rates, excessive retrieval or verifier fanout, missing staged verification, audit-heavy profile usage, and candidate budget thresholds without rerunning models or verifiers.
...
- `run_product_runtime_baseline.py` aggregates saved `ProductTrace` JSON payloads into a request-runtime baseline, including phase timing distributions, verifier-route cost summaries, retrieval-use rates, cache hit rates, staged-verification saved/verified claim counts, optional `ProductRuntimeBudgetPolicy` / `ProductPromotionContract` gate results, an artifact manifest, and an optional `product_runtime_baseline:*:*` registry record. It supports `--trace-records-jsonl` to stream per-trace metric/budget records into a sidecar instead of embedding them in the main report, `--trace-records-cache-json` to reuse compact metric/budget records across repeated budget sweeps when source trace fingerprints and the resolved policy payload match, supports `--compact-json` for minified automation artifacts, performs no model or verifier work, and is intended to validate the product control path separately from model/cache performance recommendations.
