...
- `run_product_runtime_profile_sweep.py` runs deterministic calibrated-control demo scenarios across `latency`, `balanced`, `audit`, and request-level `auto` selection modes, writes trace artifacts, builds one ProductTrace runtime baseline per mode, records actual selected profiles for `auto`, ranks non-blocked modes by request runtime and route cost, and can register the sweep report. It supports `--max-workers` mode-level concurrency while keeping trace order deterministic inside each mode; use the default single-worker mode for promotion timing evidence and multi-worker mode for faster smoke/coverage scans. It also supports `--compact-json` across generated traces, baselines, reports, and manifests without changing payload schemas. It is the lightweight local path for deciding which staged-verification mode should become the default product posture before collecting real traffic traces. Skipped verifier stages with no selected route now count as zero route cost for route-budget checks, while missing runtime traces still fail closed when runtime phase/total thresholds are configured. The current registered sweep is `report:smollm2-product-runtime-profile-sweep:0.1`, which promotes the three static profiles plus `auto` against the strict 1.4 product runtime budget and recommends `auto` on the deterministic scenario set.
