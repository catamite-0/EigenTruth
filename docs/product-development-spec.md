...
- `ProductPromotionContract` consumes a promoted release-candidate report or registry-workflow payload and produces a dependency-free product handoff containing runtime defaults, verifier-route metadata, and a `ProductRuntimeBudgetPolicy`; `calibrated_control_demo.py --promotion-contract` can use that artifact directly while still allowing explicit CLI budget overrides.
