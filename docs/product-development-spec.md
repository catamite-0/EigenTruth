...
- `run_adapter_promotion_registry_workflow.py` closes the route-baseline automation loop: it runs adapter promotion, writes the artifact manifest, recursively verifies it, and registers the promoted verifier-route baseline in `ArtifactRegistry` unless the route decision is blocked.
- `compare_route_baselines.py` compares registered verifier-route promotion manifests without rerunning model or verifier work, recursively verifies each manifest, reloads saved route-comparison reports, applies optional quality/cost/tail/retrieval gates, fails closed on invalid source metrics, and recommends one deployable route baseline.
...
- `run_local_retrieval_route_workflow.py` turns a statement-bearing score dump and local JSON/JSONL/text corpora into a registered retrieval-groundedness route baseline, including the generated claims fixture, verifier report, route comparison, promotion report, source corpus, score dump, optional persistent SQLite FTS candidate retrieval metadata, optional fingerprint-keyed claims cache metadata, and lightweight runtime profile metadata in the manifest for recursive provenance and cost checks.
- `run_adapter_readiness_workflow.py` composes adapter-family quality promotion with same-machine cache-profile performance promotion into one final fail-closed adapter readiness decision, writes `runtime-recommendation.json` from the promoted performance matrix, and includes it in the top-level artifact manifest for recursive verification and registry promotion.
...
- `profile_gate_smoke.py`, `cache_profile_smoke.py`, `cache_worker_sweep_smoke.py`, and `make perf-check` provide deterministic no-model smoke checks for profile regression, cache/cache-only gate machinery, worker-count sweep decisions, and registry-backed baselines; real performance claims still require representative `eval_truthfulqa.py --profile-json` artifacts.
- `recommend_runtime_config.py` turns promoted cache-profile matrix and optional worker-sweep reports into one deployable runtime recommendation with layer, batch size, capture mode, token budget, prefix-KV mode, worker count, best available AUROC quality signal, and equivalent benchmark flags. It performs no model work and should be treated as the final handoff from same-machine performance evidence to local deployment defaults.
- `compare_readiness_baselines.py` compares registered adapter-readiness manifests without rerunning model work: it recursively verifies each manifest, regenerates runtime recommendations from saved performance matrices when possible, applies optional quality/runtime gates, uses uncached total time as a conservative forward-cost fallback for legacy reports, and recommends the passing baseline with the best AUROC quality signal.
- `compare_release_candidates.py` combines registered readiness and verifier-route baselines into the final local release gate: both underlying baseline comparisons must promote, then the report emits one release candidate with runtime flags, verifier route, quality evidence, runtime cost, and route cost.
- `run_release_candidate_registry_workflow.py` registers a final local release candidate by writing the release-candidate report, building a manifest over that report plus the selected readiness and route manifests, recursively verifying the whole chain, and recording the verified candidate in `ArtifactRegistry` with recommended runtime, route quality, tail latency, attempted-route, and retrieval-use metadata.
