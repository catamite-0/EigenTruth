...
- `run_performance_baseline_workflow.py` wraps the runtime-recommendation handoff into a fingerprinted, registry-ready performance baseline bundle. It can run or reuse cache-profile matrix evidence, optional worker-count sweeps, optional INSIDE sampling / trigger-budget reports, then writes `performance-baseline-workflow.json`, `runtime-recommendation.json`, an artifact manifest, a top-level `performance_evidence_bundle` summary for recommendation cost ratios / evidence status / artifact readiness / score-dump cache evidence, and an optional `performance_baseline:*:*` registry record. When reusing a matrix report, the top-level config, performance evidence runtime block, and manifest metadata inherit the matrix report's effective runtime settings for reproducibility. With `--verify-manifest`, it writes a separate recursive manifest verification report and records `manifest_verification:<name>-verification:<version>` next to the performance baseline. Dry-run outputs remain `needs_evidence` so command inspection cannot be mistaken for promoted performance evidence.
