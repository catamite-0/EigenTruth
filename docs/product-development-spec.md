...
- `run_product_trace_replay_workflow.py` wraps the raw-trace handoff into one reproducible workflow: build a redacted trace corpus plus runtime-pair index, run a ProductTrace runtime baseline, run selector replay over the standardized traces using that index, recursively manifest all child reports, record phase timing/cache summaries for local performance tuning, lift the runtime baseline `optimization` status/recommendations/policy hints into the top-level report and registry metadata, optionally materialize the baseline's recommended runtime budget policy, and register a single workflow report. With `--verify-manifest`, it writes a separate recursive verification report and records `manifest_verification:<name>-verification:<version>` beside the workflow report; with `--fingerprint-cache`, repeated local manifest checks reuse unchanged artifact fingerprints across runs; with `--corpus-source-cache-json`, repeated raw-trace handoffs reuse unchanged source validation/redaction results before rebuilding corpus outputs; with `--corpus-cache-json`, repeated raw-trace handoffs reuse the already standardized corpus when source fingerprints, output fingerprints, and corpus config still match; with `--runtime-trace-records-cache-json`, repeated runtime budget sweeps reuse compact trace-record metrics; with `--save-runtime-recommended-policy`, the workflow saves a reusable `ProductRuntimeBudgetPolicy` candidate from the observed baseline; with `--selector-trace-inputs-json`, repeated selector replay sweeps reuse a minimal trace-input cache keyed by source trace fingerprints. The current registered workflow is `report:smollm2-product-trace-replay-workflow:0.1`; it promotes the default selector from 12 redacted traces with full paired-runtime coverage while keeping the runtime baseline as observed evidence unless an explicit runtime budget or promotion contract is supplied, and its verification record is `manifest_verification:smollm2-product-trace-replay-workflow-verification:0.1`.
