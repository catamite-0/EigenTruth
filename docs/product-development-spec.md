...
- `run_adapter_readiness_workflow.py` composes adapter-family quality promotion with same-machine cache-profile performance promotion into one final fail-closed adapter readiness decision and writes a top-level artifact manifest for recursive verification and registry promotion.
