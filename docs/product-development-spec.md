...
- `compare_release_candidates.py` combines registered readiness, verifier-route, optional performance-baseline records, optional selector-replay reports, optional product-runtime-drift reports, and optional adapter-family matrix evidence into the final local release gate: both underlying baseline comparisons must promote, including any readiness sampling-cost gates and route runtime/cache budget gates; a supplied `performance_baseline:*:*` key must verify, match the selected runtime recommendation, require `performance_evidence_bundle.release_ready=true` when that bundle exists, and can optionally gate score-dump cache evidence / selected JSONL view hit rate plus performance trend against an explicit prior `performance_baseline:*:*` key; supplied selector replay and product runtime drift reports must verify and promote; and a supplied adapter-family matrix must promote with all required routes present and promoted. The report emits one release candidate with runtime flags, verifier route, quality evidence, runtime cost, route cost, selected trigger-budget policy, trigger-budget metadata, ratio-source fields, performance-baseline provenance, performance evidence bundle cost/readiness/cache/trend metadata, selector replay runtime-delta evidence, product runtime drift evidence, and adapter-family coverage metadata. Optional `latency`, `balanced`, and `audit` runtime profiles fill unset performance/cost gate defaults while leaving explicit quality thresholds such as AUROC to the caller.
- `run_release_candidate_registry_workflow.py` registers a final local release candidate by writing the release-candidate report, building a manifest over that report plus the selected readiness, route, optional performance manifests, optional selector-replay manifests, optional product-runtime-drift manifests, and optional adapter-family matrix report, recursively verifying the whole chain, and recording the verified candidate in `ArtifactRegistry` with recommended runtime, selected runtime profile, selected trigger-budget policy, INSIDE sampling / trigger-budget cost evidence when available, route quality, tail latency, attempted-route, retrieval-use metadata, performance-baseline record metadata, performance evidence bundle readiness/cost/cache/trend metadata, selector replay runtime-delta metadata, product runtime drift metadata, and required adapter-family route metadata.
