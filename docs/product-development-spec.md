...
- `AdaptiveScoreTransform` / `AdaptiveConformalCalibrator` adds a dependency-free adaptive conformal scoring layer for ACSE-style experiments: callers can inflate native diagnostics with semantic entropy, cluster, prompt-risk, or verifier-route features, then save the adjusted higher-is-anomalous score as a normal calibration artifact without changing score-dump schemas.
