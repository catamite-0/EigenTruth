...
- `run_feedback_policy_workflow.py` chains the same three feedback-policy steps into one reproducible artifact bundle: feedback join report, candidate policy/default artifacts, replay audit, top-level manifest, and optional local registry record. Its top-level status is `recommend`, `observed`, `needs_evidence`, or `blocked`; it performs no model, verifier, retrieval, database, or network work.
...
- `compare_release_candidates.py` combines registered readiness, verifier-route, optional performance-baseline records, optional product-trace-replay workflow reports or `report:*:*` registry keys, optional selector-replay reports, optional product-runtime-drift reports, and optional adapter-family matrix evidence into the final local release gate: both underlying baseline comparisons must promote, including any readiness covariance-quality gates, sampling-cost gates, and route runtime/cache budget gates; a supplied `performance_baseline:*:*` key must verify, match the selected runtime recommendation, require `performance_evidence_bundle.release_ready=true` when that bundle exists, and can optionally gate covariance `maha_last` quality drop, score-dump cache evidence / selected JSONL view hit rate, plus performance trend against an explicit prior `performance_baseline:*:*` key; supplied product-trace-replay workflow, selector replay, and product runtime drift reports must verify and promote; and a supplied adapter-family matrix must promote with all required routes present and promoted. When `--product-trace-replay-workflow-key` or `--product-trace-replay-workflow` is supplied, the release gate resolves its selector-replay and runtime-drift child report paths unless explicit child paths override them. The report emits one release candidate with runtime flags, verifier route, quality evidence, runtime cost, route cost, selected trigger-budget policy, trigger-budget metadata, ratio-source fields, performance-baseline provenance, performance evidence bundle cost/readiness/cache/trend metadata, covariance tradeoff gate metadata, product-trace-replay provenance, selector replay runtime-delta evidence, product runtime drift evidence, and adapter-family coverage metadata. Optional `latency`, `balanced`, and `audit` runtime profiles fill unset performance/cost gate defaults while leaving explicit quality thresholds such as AUROC to the caller.
- `run_release_candidate_registry_workflow.py` registers a final local release candidate by writing the release-candidate report, building a manifest over that report plus the selected readiness, route, optional performance manifests, optional product-trace-replay manifests, optional selector-replay manifests, optional product-runtime-drift manifests, and optional adapter-family matrix report, recursively verifying the whole chain, and recording the verified candidate in `ArtifactRegistry` with recommended runtime, selected runtime profile, selected trigger-budget policy, covariance quality-gate metadata, INSIDE sampling / trigger-budget cost evidence when available, route quality, tail latency, attempted-route, retrieval-use metadata, performance-baseline record metadata, performance evidence bundle readiness/cost/cache/trend/score-fusion metadata, product-trace-replay metadata, selector replay runtime-delta metadata, product runtime drift metadata, and required adapter-family route metadata. `--fingerprint-cache` persists the recursive manifest fingerprint cache across repeated local release checks so unchanged source artifacts are not re-hashed, `--manifest-fingerprint-workers` enables bounded parallel artifact fingerprinting for large local manifests, `artifact_cache.artifact_fingerprint_cache` reports requests, hits, misses, entries, and hit rate, and the workflow report records compare / comparison-write / manifest-build / promotion / workflow-report-write phase timings for release-gate overhead analysis.
