...
- `run_release_candidate_registry_workflow.py` registers a final local release candidate by writing the release-candidate report, building a manifest over that report plus the selected readiness, route, optional performance manifests, optional selector-replay manifests, optional product-runtime-drift manifests, and optional adapter-family matrix report, recursively verifying the whole chain, and recording the verified candidate in `ArtifactRegistry` with recommended runtime, selected runtime profile, selected trigger-budget policy, INSIDE sampling / trigger-budget cost evidence when available, route quality, tail latency, attempted-route, retrieval-use metadata, performance-baseline record metadata, performance evidence bundle readiness/cost metadata, selector replay runtime-delta metadata, product runtime drift metadata, and required adapter-family route metadata.
