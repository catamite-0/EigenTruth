...
- `ProductTrace.to_bounded_dict()` and `calibrated_control_demo.py --bounded-trace` emit smaller online telemetry payloads by keeping diagnostics, routing decisions, summaries, selected artifact/contract metadata, and truncation counts while bounding long claim, verifier, event, and action-result lists. Bounded traces are for product telemetry; full `ProductTrace.to_dict()` payloads remain the reproduction and runtime-baseline input.
