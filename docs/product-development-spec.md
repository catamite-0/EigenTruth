...
- `ProductRuntimeBudgetPolicy` / `evaluate_product_runtime_budget` turns `ProductTrace.runtime_trace` and cache metadata into optional fail-closed product gates over total request time, named phase timings, phase p95/p99 timings, aggregate cache hit rate, and named cache hit rates; `calibrated_control_demo.py --max-runtime-total-seconds/--max-runtime-phase-seconds/--max-runtime-phase-p95-seconds/--max-runtime-phase-p99-seconds/--min-cache-hit-rate` writes the evaluated result into trace and registry metadata.
