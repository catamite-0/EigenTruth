...
- `run_cache_profile_matrix.py` expands the triplet harness across layer, batch-size, and hidden-state-capture combinations, writing a single same-machine matrix report with command logs, gate summaries, cache-only timing, `truth_proj` AUROC, and a fail-closed matrix-level performance promotion decision.
