...
- `build_product_trace_corpus.py` turns raw saved ProductTrace JSON/JSONL into replay-ready corpus artifacts: it validates risk decisions and optional runtime traces, redacts text-like claim/evidence fields by default, adds stable `metadata.runtime_replay_key` values, writes standardized trace files, builds a manifest, and can register the corpus. The current registered SmolLM2 corpus is `report:smollm2-product-trace-corpus:0.1`, with 12 accepted deterministic profile-sweep traces, no rejects, full runtime coverage, and all claim/evidence text redacted before replay.
- `run_product_trace_replay_workflow.py` wraps the raw-trace handoff into one reproducible workflow: build a redacted trace corpus, run a ProductTrace runtime baseline, run selector replay over the standardized traces, recursively manifest all child reports, and register a single workflow report. The current registered workflow is `report:smollm2-product-trace-replay-workflow:0.1`; it promotes the default selector from 12 redacted traces with full paired-runtime coverage while keeping the runtime baseline as observed evidence unless an explicit runtime budget or promotion contract is supplied.
