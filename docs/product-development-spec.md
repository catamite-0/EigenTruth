...
- `run_performance_baseline_workflow.py` wraps the runtime-recommendation handoff into a fingerprinted, registry-ready performance baseline bundle. It can run or reuse cache-profile matrix evidence, optional worker-count sweeps, optional INSIDE sampling / trigger-budget reports, then writes `performance-baseline-workflow.json`, `runtime-recommendation.json`, an artifact manifest, and an optional `performance_baseline:*:*` registry record. Dry-run outputs remain `needs_evidence` so command inspection cannot be mistaken for promoted performance evidence.
