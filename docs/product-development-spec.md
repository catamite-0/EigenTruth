...
- `compare_verifier_routes.py` aggregates route-level verifier quality, runtime cost, and control impact across saved verifier-ensemble reports, producing a cost-aware route leaderboard, Pareto frontier candidates, by-route weighted metrics, and optional fail-closed route quality gates without loading models.
...
- Extend route-level metrics to future adapters and real fixtures, then compare them with `compare_verifier_routes.py` so new tools are judged by route hit rate, false support, false refutation, runtime cost, downstream conformal control impact, Pareto frontier status, and explicit promotion gates before adapter work proceeds.
