...
- `production_tool_loop_demo.py` emits one product-style trace that combines SQLite pre-tool checks, mapped local tool output, post-tool structured verification, and runtime route summary metadata.
...
- Add real side-effecting executors behind the same trace and tool-output mapping interfaces; the current production-like demo intentionally ingests local JSON output instead of performing a live mutation.
