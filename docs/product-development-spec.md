...
- `compare_product_runtime_baselines.py` compares a fresh ProductTrace runtime baseline against a file or registered `product_runtime_baseline:*:*` record, writing a manifest-backed drift report with fail-closed gates for request latency, route duration, attempted-route count, retrieval-use drift, cache-hit-rate drop, verifier-skip-rate drop, and minimum current trace count. It records optional `product_runtime_drift_report:*:*` registry metadata and gives the product loop a local continuous-assurance check before selector or release-candidate defaults are trusted on shifted traffic.
