...
- `run_feedback_policy_workflow.py` chains the same three feedback-policy steps into one reproducible artifact bundle: feedback join report, candidate policy/default artifacts, replay audit, top-level manifest, and optional local registry record. Its top-level status is `recommend`, `observed`, `needs_evidence`, or `blocked`; registered `report:<name>:<version>` records can be consumed by release-candidate gates; it performs no model, verifier, retrieval, database, or network work.
