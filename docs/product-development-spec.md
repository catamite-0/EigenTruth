...
- `compare_release_candidates.py` combines registered readiness, verifier-route, optional performance-baseline records, optional selector-replay reports, optional product-runtime-drift reports, and optional adapter-family matrix evidence into the final local release gate: both underlying baseline comparisons must promote, including any readiness sampling-cost gates and route runtime/cache budget gates; a supplied `performance_baseline:*:*` key must verify, match the selected runtime recommendation, and require `performance_evidence_bundle.release_ready=true` when that bundle exists; supplied selector replay and product runtime drift reports must verify and promote; and a supplied adapter-family matrix must promote with all required routes present and promoted. The report emits one release candidate with runtime flags, verifier route, quality evidence, runtime cost, route cost, selected trigger-budget policy, trigger-budget metadata, ratio-source fields, performance-baseline provenance, performance evidence bundle cost/readiness metadata, selector replay runtime-delta evidence, product runtime drift evidence, and adapter-family coverage metadata. Optional `latency`, `balanced`, and `audit` runtime profiles fill unset performance/cost gate defaults while leaving explicit quality thresholds such as AUROC to the caller.
