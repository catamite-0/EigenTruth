...
- `run_product_trace_replay_workflow.py` wraps the raw-trace handoff into one reproducible workflow: build a redacted trace corpus plus runtime-pair index, run a ProductTrace runtime baseline, run selector replay over the standardized traces using that index, recursively manifest all child reports, and register a single workflow report. The current registered workflow is `report:smollm2-product-trace-replay-workflow:0.1`; it promotes the default selector from 12 redacted traces with full paired-runtime coverage while keeping the runtime baseline as observed evidence unless an explicit runtime budget or promotion contract is supplied.
