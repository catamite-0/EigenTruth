...
- `run_product_runtime_profile_sweep.py` runs deterministic calibrated-control demo scenarios across `latency`, `balanced`, and `audit` runtime profiles, writes trace artifacts, builds one ProductTrace runtime baseline per profile, ranks non-blocked profiles by request runtime and route cost, and can register the sweep report. It is the lightweight local path for deciding which staged-verification profile should become the default product posture before collecting real traffic traces.
