...
- `run_calibrated_observability_workflow.py` now wraps score-dump creation or reuse plus conformal report, sweep report, best calibration artifact, nested manifest verification, top-level artifact manifest, and optional registry record into one reproducible calibrated-observability closure; `--runtime-preset quick|calibrate|full` fixes common local smoke, dump-reuse, and real-run parameter profiles while preserving explicit CLI overrides.
