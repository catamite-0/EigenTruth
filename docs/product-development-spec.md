...
- `ScoreDump` / `load_score_dump` / `score_dump_file_metadata` provide one validated post-hoc score-dump entry point for conformal calibration, score-ensemble, verifier-ensemble, fixture-builder, calibration-transfer, and sweep-calibration reports, including record counts, score names, sweep-layer metadata, file size, and SHA-256 provenance so repeated analyses can verify exact input reuse without model reruns. `LayerScoreSweepCalibrator.calibrate_from_score_dump()` consumes validated dumps directly, report builders share a run-local metadata cache to avoid duplicate hashing of repeated score paths, and `ScoreDumpJsonlManifest` / `ScoreDumpRecord` / `iter_score_dump_jsonl_records()` provide an optional manifest-backed JSONL format for large post-hoc score artifacts.
