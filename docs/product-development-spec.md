...
- Local release-candidate smoke artifact: `benchmark_manifest:tiny-local-staged-qa-release-candidate:0.4` combines the tiny-gpt2 offline readiness/runtime baseline with the TruthfulQA l80 staged structured QA route baseline. It proves the registry/release-gate plumbing end to end; representative Qwen or SmolLM2 readiness artifacts are still required before treating it as a model-specific deployment recommendation.
