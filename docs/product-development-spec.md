...
- `refresh_verifier_route_artifacts.py` regenerates current-schema verifier route reports from saved score dumps, claims, and local verifier corpora so old artifacts can be upgraded for promotion gates without rerunning model forward passes; its summary exposes route latency/cost/cache metrics for promotion audit, and regression coverage now includes both structured QA and structured-state promotion gates.
