...
- `run_world_model_signal_calibration_workflow.py` specializes that bridge for world-model correction: it builds a deterministic state-transition fixture, can inject a controlled multi-world-model ensemble disagreement fixture, runs the world-model verifier route, emits verifier/world-model score columns including nonzero agreement-gap signals, evaluates score and geometry fusion, recursively verifies manifests, and can register the workflow as a local report. Its `policy_replay` ensemble strategy makes disagreement arise from a conservative high-quantity reservation policy instead of direct label shaping, providing a local stand-in for future learned/simulator world-model replay.
