...
2. Collect or derive the explicit inputs requested by
   `run_world_model_rule_authoring_adapter.py`, then promote only the executed
   deterministic calculator/entity-role/world-model results that pass a
   provenance gate. The complete source-backed replay now proves local-catalog
