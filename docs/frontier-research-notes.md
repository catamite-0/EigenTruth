...
- High-certainty hallucination work such as CHOKE / "LLMs Hallucinate with Certainty Despite Knowing the Answer" (arXiv:2502.12964, EMNLP Findings 2025) is a warning against entropy-only or confidence-only product gates. EigenTruth should continue requiring independent verifier, retrieval, counterfactual, or world-model evidence for release promotion, and should treat certainty signals as one calibrated axis rather than proof of factuality. The local bridge is now `PerturbationConsistencyVerifier`: answer-preserving prompt variants supplied by the caller are checked at triple level against the anchor claim, and `build_verifier_signal_score_dump.py` can expose conflict, high-confidence conflict, missing, failed, and not-applicable rates as ordinary calibration/fusion signals.
...
- Trace-provenance summaries now roll up into runtime baselines, product runtime drift gates, replay workflow metadata, an optional release-candidate requirement via `--require-product-runtime-drift-provenance-evidence`, and registry workflow metadata when that final gate is enabled, covering claim/evidence/action-result/final-answer graph coverage plus missing-reference and unsupported-supported-claim rates.
...
- `benchmarks/summarize_unresolved_frontier_evidence.py` now makes the remaining closure state explicit in one non-evidence artifact: source-family acquisition is covered for the v4 unresolved queue, citation/search alignment is still blocked, and world-model rule coverage is partial because the promoted `9/9` mechanism bundle covers causal/procedural tasks while numeric and temporal rule inputs still need source-backed fills. Release candidates can now require this summary as a closure guard through `--require-unresolved-frontier-evidence-closure`; the guard blocks when the summary is absent, non-promoted, or still has open `next_actions`, but it does not turn unresolved summaries into verifier evidence.
- The unresolved closure summary now subtracts promoted rule-family counts from the original world-model input plan and consumes both the current rule-input audit and completed requeue artifacts before emitting next actions. An earlier registered summary at `artifacts/frontier-release-evidence/unresolved-frontier-evidence-summary-v1/` remained `needs_evidence` because citation/search alignment was still blocked and world-model rule inputs were partial, but its world-model lane was no longer `needs_requeue`: `causal_or_procedural=9` was closed by the promoted mechanism bundle, raw remaining work was `quantity_or_arithmetic=12` plus `temporal_consistency=5`, the audit report at `artifacts/frontier-release-evidence/unresolved-world-model-rule-input-plan-audit-v1/` marked `8` rows for entity-role requeue, and `artifacts/frontier-release-evidence/unresolved-world-model-rule-stub-requeue-v1/` applied all `8/8` suggestions with `0` skipped. The requeued adapter/input-plan chain at `artifacts/frontier-release-evidence/unresolved-world-model-rule-requeued-authoring-adapter-v1/` and `artifacts/frontier-release-evidence/unresolved-world-model-rule-requeued-input-plan-v1/` yielded `8` entity-disambiguation tasks requiring `subject_entity`, `answer_entity`, `requested_role`, `expected_entity`, and `source_citation`; that intermediate state narrowed `next_actions` to citation alignment plus source-backed fill/promotion of the remaining world-model rules. The 2026-07-03 closure refresh supersedes that state: the checked-in summary now promotes with `next_actions=[]`, covered-fact route coverage `1.0`, coverage gap `0`, and closure verification `pass`, while blocked citation/search query-sweep diagnostics remain preserved as negative evidence.
...
- `LearnedPreGenerationRiskEstimate.from_probe(...)` bridges saved `AttentionSoftTargetProbeArtifact` objects into the same product pre-generation assessment boundary, recording a JSON-ready score, probability, risk level, source, layer, optional attention summary, and metadata for one prompt.
...
- The default policy records both transparent soft estimates and learned probe estimates without changing the existing hard-rule profile route; `route_on_soft_risk=true` or `route_on_learned_risk=true` can explicitly upgrade low hard-rule prompts into `balanced` or `audit`.
- This is the product-control shell for the pre-generation soft-target direction: it makes risk-estimation traces and routing semantics stable now, while detector-quality claims still require calibrated larger-model runs and release evidence.
...
- `ProductTrace.citation_integrity_summary()`, bounded `summaries.citation_integrity`, `product_runtime_metrics(...)`, and `run_product_runtime_baseline.py` now expose cited-claim coverage, catalog mismatch/unresolved counts, trace gaps, status/rule/source counts, plus manifest/registry metadata. `compare_product_runtime_baselines.py` can fail closed on citation-integrity participation/coverage drops and mismatch/unresolved/issue/trace-gap drift, `run_product_trace_replay_workflow.py` now passes those gates through as part of the end-to-end replay handoff, `compare_release_candidates.py --require-product-runtime-drift-citation-integrity-evidence` can require that evidence at the final local release gate, and `run_release_candidate_registry_workflow.py` preserves the required flag plus citation-integrity metric counts and per-metric statuses in registered candidate metadata. This turns CiteCheck-style citation metadata drift into product telemetry and release-gate evidence without adding network retrieval or a database.
...
- `build_verifier_signal_score_dump.py` converts `eval_verifier_ensemble.py --verified-records-jsonl` sidecars into standard score columns such as `verifier_refuted`, `verifier_refute_confidence`, `verifier_uncertainty`, `selfcheck_refute_rate`, `fact_selfcheck_refute_rate`, `fact_selfcheck_uncovered_rate`, `perturbation_conflict_rate`, `perturbation_high_confidence_conflict_rate`, `world_model_disagreement`, `world_model_conflict`, and `world_model_conflict_delta`. The world-model bridge now reads state-transition `prediction_metadata`, explicit `world_model_conflict` metadata, and direct ensemble-verifier agreement metadata, the fact-selfcheck bridge reads triple-level support/refute/uncovered rates, and the perturbation bridge reads CHOKE-style prompt-variant consistency reports so claim-level sampling evidence is not silently dropped before conformal calibration.
...
- `fill_world_model_rule_inputs_from_temporal_bindings.py` closes the temporal
...
- `bind_frontier_research_queue_seed_inputs.py` closes the source-family seed
  handoff: when the URL seed sidecar is audit-ready, it stages the
  `source-family-url-seeds.jsonl` path back into frontier command bindings and
...
- The checked-in unresolved summary was first refreshed with that semantic:
  `22/88` unresolved targets were covered by the promoted scoped route, leaving
  ...
  remained promoted/covered. That intermediate command queue has now been
  executed and verified: the 2026-07-03 summary records covered-fact route
  coverage `1.0`, coverage gap `0`, closure verification `pass`, and no open
  `next_actions`; the derived frontier command plan is `empty`.
