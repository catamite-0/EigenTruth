...
- Two Pathways to Truthfulness (arXiv:2601.07422, accepted ACL 2026 Main Conference) argues that internal truthfulness cues split into a question-anchored pathway using question-answer information flow and an answer-anchored pathway using evidence from the generated answer itself. EigenTruth now implements dependency-free prompt/answer hidden-state pathway proxies and an optional attention-flow pathway readout from returned model attentions. This is not a mechanistic attention-knockout or token-patching replication; it gives the score-dump/conformal/fusion pipeline cheap pathway-disagreement and pathway-flow signals before heavier causal-intervention experiments.
