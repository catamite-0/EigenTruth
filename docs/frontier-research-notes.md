...
- Two Pathways to Truthfulness (arXiv:2601.07422, accepted ACL 2026 Main Conference) argues that internal truthfulness cues split into a question-anchored pathway using question-answer information flow and an answer-anchored pathway using evidence from the generated answer itself. EigenTruth now implements a dependency-free, training-free prompt/answer pathway proxy from existing forced-answer hidden states. This is not a mechanistic attention-knockout or token-patching replication; it gives the score-dump/conformal/fusion pipeline a cheap pathway-disagreement signal before heavier causal-intervention experiments.
