...
- The unresolved closure summary now subtracts promoted rule-family counts from the original world-model input plan and consumes the current rule-input audit before emitting next actions. The current registered summary at `artifacts/frontier-release-evidence/unresolved-frontier-evidence-summary-v1/` remains `needs_evidence`: `causal_or_procedural=9` is closed by the promoted mechanism bundle, raw remaining work is `quantity_or_arithmetic=12` plus `temporal_consistency=5`, and the audit report at `artifacts/frontier-release-evidence/unresolved-world-model-rule-input-plan-audit-v1/` marks `8` rows as `needs_requeue` into `entity_disambiguation`. The action-facing view is now `entity_disambiguation=8`, `quantity_or_arithmetic=6`, and `temporal_consistency=3`, with required input counts emitted separately from the raw closure ledger.
