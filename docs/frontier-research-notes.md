...
- `ProductTrace.citation_integrity_summary()`, bounded `summaries.citation_integrity`, `product_runtime_metrics(...)`, and `run_product_runtime_baseline.py` now expose cited-claim coverage, catalog mismatch/unresolved counts, trace gaps, status/rule/source counts, plus manifest/registry metadata. `compare_product_runtime_baselines.py` can fail closed on citation-integrity participation/coverage drops and mismatch/unresolved/issue/trace-gap drift, and `run_product_trace_replay_workflow.py` now passes those gates through as part of the end-to-end replay handoff. This turns CiteCheck-style citation metadata drift into product telemetry and release-gate evidence without adding network retrieval or a database.
