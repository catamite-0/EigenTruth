...
- The unresolved closure summary now subtracts promoted rule-family counts from the original world-model input plan and consumes both the current rule-input audit and completed requeue artifacts before emitting next actions. The current registered summary at `artifacts/frontier-release-evidence/unresolved-frontier-evidence-summary-v1/` remains `needs_evidence` because citation/search alignment is still blocked and world-model rule inputs remain partial, but its world-model lane is no longer `needs_requeue`: `causal_or_procedural=9` is closed by the promoted mechanism bundle, raw remaining work is `quantity_or_arithmetic=12` plus `temporal_consistency=5`, the audit report at `artifacts/frontier-release-evidence/unresolved-world-model-rule-input-plan-audit-v1/` marks `8` rows for entity-role requeue, and `artifacts/frontier-release-evidence/unresolved-world-model-rule-stub-requeue-v1/` applies all `8/8` suggestions with `0` skipped. The requeued adapter/input-plan chain at `artifacts/frontier-release-evidence/unresolved-world-model-rule-requeued-authoring-adapter-v1/` and `artifacts/frontier-release-evidence/unresolved-world-model-rule-requeued-input-plan-v1/` yields `8` entity-disambiguation tasks requiring `subject_entity`, `answer_entity`, `requested_role`, `expected_entity`, and `source_citation`; summary `next_actions` now correctly narrows to citation alignment plus source-backed fill/promotion of the remaining world-model rules.
