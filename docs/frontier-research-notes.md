...
- Two Pathways to Truthfulness (arXiv:2601.07422, accepted ACL 2026 Main Conference) argues that internal truthfulness cues split into a question-anchored pathway using question-answer information flow and an answer-anchored pathway using evidence from the generated answer itself. EigenTruth now implements dependency-free prompt/answer hidden-state pathway proxies, an optional attention-flow pathway readout from returned model attentions, and local pathway-intervention analysis helpers for prompt/answer attention knockout plus before/after score-effect accounting. This is still not a full model-rerun token-patching replication; it gives the score-dump/conformal/fusion pipeline cheap pathway-disagreement, pathway-flow, and mechanism-experiment scaffolding before heavier causal-intervention runs.
