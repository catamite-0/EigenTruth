...
2. Build a source-backed handoff for the promoted rule candidate, then expand
   `fill_world_model_rule_inputs_from_correction_handoff.py` or a sibling
   adapter to fill more typed batches with provenance-backed values. Only
   deterministic calculator/entity-role/world-model results that pass the
   promotion gate should enter correction handoff. The complete source-backed
