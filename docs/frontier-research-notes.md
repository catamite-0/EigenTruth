...
2. Implement the deterministic rule/world-model executor for the newly
   materialized rule-authoring stubs. The complete source-backed replay now
   proves local-catalog coverage is not enough (`0/88` mapped), so the highest
   value code step is executing calculator/entity-role/causal rule stubs with
