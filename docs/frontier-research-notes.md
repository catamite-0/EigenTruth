...
- `run_world_model_signal_calibration_workflow.py` specializes that bridge for world-model correction: it builds a deterministic state-transition fixture, runs the world-model verifier route, emits verifier/world-model score columns, evaluates score and geometry fusion, recursively verifies manifests, and can register the workflow as a local report.
