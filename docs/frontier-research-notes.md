...
- The unresolved closure summary now subtracts promoted rule-family counts from the original world-model input plan before emitting next actions. The current registered summary at `artifacts/frontier-release-evidence/unresolved-frontier-evidence-summary-v1/` remains `needs_evidence`, but the world-model lane is now precise: `causal_or_procedural=9` is closed by the promoted mechanism bundle, while the remaining rule work is `quantity_or_arithmetic=12` and `temporal_consistency=5`. This keeps future input-fill plans pointed at numeric/temporal source-backed rows instead of stale mechanism fields.
