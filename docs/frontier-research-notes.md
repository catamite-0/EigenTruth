...
2. Fill the typed batches from `build_world_model_rule_input_collection_plan.py`
   with provenance-backed values, rerun
   `run_world_model_rule_authoring_adapter.py` with the resulting rule-input
   file, then promote only deterministic calculator/entity-role/world-model
   results that pass a provenance gate. The complete source-backed replay now
