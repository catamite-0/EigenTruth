"""Fill mechanism rule inputs from explicit source-backed bindings.

This workflow is a narrow bridge for causal/procedural rule tasks. It does not
infer mechanisms, run citation search, or judge claims by itself. Callers must
provide JSONL binding rows that explicitly state the mechanism, precondition,
mechanism status, and source citation. Filled rows remain adapter inputs and
still require deterministic execution plus promotion before product handoff.
"""
...
WORKFLOW = "world_model_rule_input_mechanism_binding_fill"
SOURCE_WORKFLOW = "world_model_rule_input_collection_plan"
MECHANISM_COLLECTION_FAMILY = "mechanism_rule_input_collection"
...
def fill_world_model_rule_inputs_from_mechanism_bindings(
    *,
    input_tasks: Sequence[Mapping[str, Any]],
    mechanism_bindings: Sequence[Mapping[str, Any]],
    metadata: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    """Return filled mechanism rule-input rows plus fail-closed diagnostics."""
    bindings_by_request: dict[str, Mapping[str, Any]] = {}
    duplicate_binding_ids: set[str] = set()
    for binding in mechanism_bindings:
    ...
    for task in input_tasks:
        ...
        if str(task.get("collection_family") or "") != MECHANISM_COLLECTION_FAMILY:
            unfilled.append(_unfilled(task, reason="unsupported_collection_family"))
            continue
        binding = bindings_by_request.get(request_id)
        if binding is None:
            unfilled.append(_unfilled(task, reason="missing_mechanism_binding"))
            continue
        unused_bindings.discard(request_id)
        failures = _binding_failures(binding, duplicate=request_id in duplicate_binding_ids)
        if failures:
            unfilled.append(_unfilled(task, reason="invalid_mechanism_binding", failures=failures))
            continue
        filled.append(_filled_mechanism_input(task, binding=binding))
    ...
    return {
        ...
        "scope": (
            "Fills explicit causal/procedural mechanism rule inputs from caller-supplied "
            "source-backed bindings. Filled rows are adapter inputs, not verifier evidence, "
            "and still require adapter execution plus promotion before product handoff."
        ),
        "source": {
        ...
        "label_usage": {
            ...
            "mechanism_status_bound_to_rule_inputs": True,
        ...
        "skipped_mechanism_bindings": skipped_bindings,
...
def run(
    *,
    ...
    mechanism_bindings_path: str | Path,
    output_dir: str | Path,
    ...
    """Fill, write, manifest, and optionally register mechanism rule-input rows."""
    if registry_path is not None and (not name or not version):
        raise ValueError("registry_path requires name and version.")
    output = Path(output_dir)
    output.mkdir(parents=True, exist_ok=True)
    report_path = Path(report_json_path or output / "rule-input-mechanism-binding-fill.json")
    ...
    skipped_path = Path(skipped_bindings_path or output / "skipped-mechanism-bindings.jsonl")
    ...
    mechanism_bindings = _load_jsonl_mappings(mechanism_bindings_path)
    payload = fill_world_model_rule_inputs_from_mechanism_bindings(
        input_tasks=input_tasks,
        mechanism_bindings=mechanism_bindings,
        metadata=metadata,
    )
    ...
    payload["paths"] = {
        ...
        "mechanism_bindings": str(mechanism_bindings_path),
        "report": str(report_path),
        "rule_inputs": str(inputs_path),
        "unfilled_tasks": str(unfilled_path),
        "skipped_mechanism_bindings": str(skipped_path),
    ...
    _write_jsonl(skipped_path, payload["skipped_mechanism_bindings"], compact=compact_json)
    manifest = build_artifact_manifest(
        {
            "rule_input_mechanism_binding_fill": report_path,
            "rule_inputs": inputs_path,
            "unfilled_rule_input_tasks": unfilled_path,
            "skipped_mechanism_bindings": skipped_path,
            "rule_input_tasks": Path(input_tasks_path),
            "mechanism_bindings": Path(mechanism_bindings_path),
        },
...
def _filled_mechanism_input(task: Mapping[str, Any], *, binding: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": 1,
        "workflow": WORKFLOW,
        ...
        "rule_family": str(task.get("rule_family") or "causal_or_procedural"),
        "mechanism": _clean(binding.get("mechanism")),
        "precondition": _clean(binding.get("precondition")),
        "mechanism_status": _normalize_mechanism_status(binding.get("mechanism_status")),
        ...
        "confidence": _optional_float(binding.get("confidence"), default=0.95),
        ...
        "provenance": {
            ...
            "mechanism_source": str(binding.get("mechanism_source") or ""),
            "precondition_source": str(binding.get("precondition_source") or ""),
            "mechanism_status_source": str(binding.get("mechanism_status_source") or ""),
...
def _binding_failures(binding: Mapping[str, Any], *, duplicate: bool) -> tuple[str, ...]:
    failures = []
    if duplicate:
        failures.append("duplicate_mechanism_binding")
    ...
    if _clean(binding.get("mechanism_status")) and _normalize_mechanism_status(binding.get("mechanism_status")) is None:
...
def _summary(
    *,
    ...
    review_status_counts = Counter(str(item.get("review_status") or "") for item in mechanism_bindings)
    mechanism_status_counts = Counter(
        _normalize_mechanism_status(item.get("mechanism_status")) or "invalid_or_missing"
        for item in mechanism_bindings
    )
    return {
        "input_task_count": len(input_tasks),
        "mechanism_binding_count": len(mechanism_bindings),
        ...
        "mechanism_status_counts": _sorted_counter(mechanism_status_counts),
...
def _normalize_mechanism_status(value: Any) -> str | None:
    ...
    if not status:
        return None
    return MECHANISM_STATUS_ALIASES.get(status)
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    parser.add_argument("--mechanism-bindings", required=True)
    ...
    payload = run(
        ...
        mechanism_bindings_path=args.mechanism_bindings,
    ...
    print(
        "world_model_rule_input_mechanism_binding_fill_ok "
