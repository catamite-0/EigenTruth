"""Fill entity-role rule inputs from explicit source-backed bindings.

This workflow is a narrow bridge for requeued entity-disambiguation rule tasks.
It does not discover facts or infer answers by itself. Callers must provide
JSONL binding rows that explicitly name the candidate answer entity, the
source-backed expected entity/value, the requested role, and a source citation.
Rows that lack provenance or do not match a pending task remain unfilled.
"""
...
WORKFLOW = "world_model_rule_input_entity_binding_fill"
SOURCE_WORKFLOW = "world_model_rule_input_collection_plan"
ENTITY_COLLECTION_FAMILY = "entity_role_rule_input_collection"
RESERVED_FIELDS = {"answer", "answers", "is_false", "label", "labels", "model_answer", "score_label"}


def fill_world_model_rule_inputs_from_entity_bindings(
    *,
    input_tasks: Sequence[Mapping[str, Any]],
    entity_bindings: Sequence[Mapping[str, Any]],
    metadata: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    """Return filled rule-input rows plus fail-closed diagnostics."""
    ...
    for task in input_tasks:
        ...
        if str(task.get("collection_family") or "") != ENTITY_COLLECTION_FAMILY:
            unfilled.append(_unfilled(task, reason="unsupported_collection_family"))
            continue
        binding = bindings_by_request.get(request_id)
        if binding is None:
            unfilled.append(_unfilled(task, reason="missing_entity_binding"))
            continue
        unused_bindings.discard(request_id)
        failures = _binding_failures(binding, duplicate=request_id in duplicate_binding_ids)
        if failures:
            unfilled.append(_unfilled(task, reason="invalid_entity_binding", failures=failures))
            continue
        filled.append(_filled_entity_role_input(task, binding=binding))
    ...
    return {
        ...
        "scope": (
            "Fills explicit entity-role deterministic rule inputs from caller-supplied "
        ...
        "label_usage": {
            ...
            "candidate_answers_bound_to_rule_inputs": True,
            "source_backed_expected_entities_required": True,
        ...
        "skipped_entity_bindings": skipped_bindings,
...
def run(
    *,
    ...
    report_path = Path(report_json_path or output / "rule-input-entity-binding-fill.json")
    ...
    skipped_path = Path(skipped_bindings_path or output / "skipped-entity-bindings.jsonl")
    ...
    payload = fill_world_model_rule_inputs_from_entity_bindings(
        input_tasks=input_tasks,
        entity_bindings=entity_bindings,
        metadata=metadata,
    )
    ...
    payload["paths"] = {
        ...
        "skipped_entity_bindings": str(skipped_path),
    ...
    _write_jsonl(skipped_path, payload["skipped_entity_bindings"], compact=compact_json)
    manifest = build_artifact_manifest(
        {
            "rule_input_entity_binding_fill": report_path,
            "rule_inputs": inputs_path,
            "unfilled_rule_input_tasks": unfilled_path,
            "skipped_entity_bindings": skipped_path,
...
def _filled_entity_role_input(task: Mapping[str, Any], *, binding: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": 1,
        "workflow": WORKFLOW,
        ...
        "answer_entity": _clean(binding.get("answer_entity")),
        "expected_entity": _clean(binding.get("expected_entity")),
        "requested_role": _clean(binding.get("requested_role")),
        ...
        "provenance": {
            ...
            "candidate_answer_source": str(binding.get("candidate_answer_source") or ""),
            "expected_entity_source": str(binding.get("expected_entity_source") or ""),
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    print(
        "world_model_rule_input_entity_binding_fill_ok "
