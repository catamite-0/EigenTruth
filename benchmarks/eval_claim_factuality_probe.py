"""Train/evaluate a claim-level factuality probe from local hidden states."""
...
CLAIM_FACTUALITY_RISK_SCORE_NAME = "claim_factuality_risk_probability"
CLAIM_FACTUALITY_SWEEP_BEST_BY = (
    "auto",
    "label_auroc",
    "target_bce",
    "target_mse",
...
class ClaimFactualityProbeRecord:
    """One claim-level hidden-state record for factuality probe training."""
    ...
    def __post_init__(self) -> None:
        ...
        token_span = _token_span(self.token_span, seq_len=hidden_states.shape[0])
        if token_span is not None and not bool(attention_mask[token_span[0]:token_span[1]].any().item()):
            raise ValueError("token_span must contain at least one unmasked token.")
        risk_target = _probability_float(self.risk_target, field_name="risk_target")
        ...
        object.__setattr__(self, "risk_target", risk_target)
    ...
    def from_mapping(
        ...
        *,
        ...
        risk_target = _first_present(payload, ("risk_target", "soft_target"))
        if risk_target is None:
            if label is None:
                raise ValueError(f"record {index} must provide risk_target, soft_target, label, or is_false.")
            risk_target = _binary_label(label, field_name="label")
        record_id = _first_present(payload, ("id", "record_id", "claim_id"))
        return cls(
            record_id=f"claim-r{index}" if record_id is None else str(record_id),
            ...
            token_span=_optional_token_span(payload.get("token_span")),
            selected_layer=selected_layer,
            claim_id=None if payload.get("claim_id") is None else str(payload.get("claim_id")),
            text=None if payload.get("text") is None else str(payload.get("text")),
            metadata=payload.get("metadata", {}),
        )


def run_claim_factuality_probe_eval(
    ...
    *,
    ...
    """Train/evaluate a claim factuality-risk probe from local hidden-state records."""
    records_path = Path(records_path)
    records = load_claim_factuality_probe_records(records_path, record_layer=record_layer)
    ...
    train_hidden, train_mask, train_spans, train_targets, _train_labels = _stack_records(records, train_indices)
    artifact = ClaimFactualityProbeArtifact.fit(
        train_hidden,
        train_targets,
        token_spans=train_spans,
        ...
        seed=seed,
        metadata={
            "workflow": "claim_factuality_probe_eval",
    ...
    conformal_report = _claim_factuality_conformal_report(
        artifact,
        records,
    ...
    if calibration_path is not None:
        ...
        calibration_artifact = _build_claim_factuality_calibration_artifact(
...
def run_claim_factuality_probe_layer_sweep(
    records_path: str | Path,
    *,
    ...
    """Train/evaluate one claim factuality probe per selected record layer."""
    records_path = Path(records_path)
    layers = (
    ...
    if best_by not in CLAIM_FACTUALITY_SWEEP_BEST_BY:
        raise ValueError(f"best_by must be one of {CLAIM_FACTUALITY_SWEEP_BEST_BY}.")

    candidates: list[dict[str, Any]] = []
    for layer in layers:
        candidate_payload = run_claim_factuality_probe_eval(
            records_path,
    ...
    if recommended is not None and (artifact_path is not None or calibration_path is not None):
        saved_recommended = run_claim_factuality_probe_eval(
            records_path,
    ...
    payload = {
        "workflow": "claim_factuality_probe_layer_sweep",
        "records_path": str(records_path),
...
def load_claim_factuality_probe_records(
    path: str | Path,
    *,
    record_layer: int | None = None,
) -> tuple[ClaimFactualityProbeRecord, ...]:
    """Load claim factuality probe records from JSON or JSONL."""
    path = Path(path)
    if path.suffix.lower() == ".jsonl":
        ...
        with path.open(encoding="utf-8") as f:
            for line_number, line in enumerate(f, start=1):
                ...
                records.append(
                    ClaimFactualityProbeRecord.from_mapping(
                        payload,
                        index=len(records),
                        record_layer=record_layer,
                    )
    ...
    for index, item in enumerate(raw_records):
        ...
        records.append(ClaimFactualityProbeRecord.from_mapping(item, index=index, record_layer=record_layer))
    return tuple(records)
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.sweep_layers is not None:
        return run_claim_factuality_probe_layer_sweep(
            args.records,
    ...
    return run_claim_factuality_probe_eval(
        args.records,
        output_path=args.json,
        artifact_path=args.save_artifact,
        calibration_path=args.save_calibration,
...
def _claim_factuality_conformal_report(
    artifact: ClaimFactualityProbeArtifact,
    records: Sequence[ClaimFactualityProbeRecord],
    *,
    ...
    if label_result is None:
        return {
            ...
            "reason": (
                ...
                "to derive a calibration label from risk_target"
            ),
            "score_name": CLAIM_FACTUALITY_RISK_SCORE_NAME,
            "direction": "higher",
            "alpha": alpha_value,
            "label_source": None,
    ...
    train_tensor = torch.as_tensor(tuple(train_indices), dtype=torch.int64)
    test_tensor = torch.as_tensor(tuple(test_indices), dtype=torch.int64)
    train_scores = probabilities[train_tensor]
    test_scores = probabilities[test_tensor]
    train_false = false_labels[train_tensor]
    test_false = false_labels[test_tensor]
    ...
    return {
        ...
        "split_reports": {
            ...
            "all": _selective_probe_report(probabilities, false_labels, threshold, alpha_value),
        },
        "record_scores": {
...
def _build_claim_factuality_calibration_artifact(
    *,
    records_path: Path,
    threshold: float,
    alpha: float,
    ...
    return CalibrationArtifact(
        ...
        scores=(
            CalibrationScore(
                name=CLAIM_FACTUALITY_RISK_SCORE_NAME,
                threshold=threshold,
...
def _hidden_states_from_payload(
    ...
    *,
    ...
    layer_payload = _first_present(payload, ("layer_hidden_states", "claim_hidden_states_by_layer"))
    if isinstance(layer_payload, Mapping):
        ...
        direct = _first_present(payload, ("hidden_states", "claim_hidden_states", "answer_hidden_states"))
...
def _infer_record_layer(records: Sequence[ClaimFactualityProbeRecord]) -> int | None:
...
def _evaluate_split(
    ...
    hidden, mask, spans, targets, labels = _stack_records(records, indices)
    with torch.no_grad():
        probabilities = artifact.predict_proba(hidden, token_spans=spans, attention_mask=mask)
...
def _record_probabilities(
    ...
    hidden, mask, spans, _targets, _labels = _stack_records(records, tuple(range(len(records))))
    with torch.no_grad():
        return artifact.predict_proba(hidden, token_spans=spans, attention_mask=mask).detach().cpu().to(torch.float64)
...
def _false_labels_for_records(
    ...
    *,
    ...
    return (
        torch.tensor([1 if record.risk_target > cutoff else 0 for record in records], dtype=torch.int64),
        "soft_target_cutoff",
    )
    ...
    return tuple(
        {
            ...
            "claim_id": records[int(index)].claim_id,
            "probability": float(probabilities[row].item()),
            "label": int(false_labels[row].item()),
            "risk_target": records[int(index)].risk_target,
        }
...
def _stack_records(
    records: Sequence[ClaimFactualityProbeRecord],
    indices: Sequence[int],
) -> tuple[torch.Tensor, torch.Tensor, tuple[tuple[int, int], ...], torch.Tensor, torch.Tensor | None]:
    if not indices:
    ...
    targets = torch.tensor([record.risk_target for record in selected], dtype=torch.float32)
    labels: list[int] = []
    all_labeled = True
    for row, record in enumerate(selected):
        ...
        spans.append(record.token_span if record.token_span is not None else (0, seq_len))
        if record.label is None:
            all_labeled = False
        else:
            labels.append(int(record.label))
    return hidden, mask, tuple(spans), targets, torch.tensor(labels, dtype=torch.int64) if all_labeled else None
...
def _optional_token_span(value: Any) -> tuple[int, int] | None:
    if value is None:
        return None
    if not isinstance(value, Sequence) or isinstance(value, (str, bytes)) or len(value) != 2:
        raise ValueError("token_span must contain exactly two integer offsets.")
    return (int(value[0]), int(value[1]))


def _token_span(value: tuple[int, int] | None, *, seq_len: int) -> tuple[int, int] | None:
    if value is None:
        return None
    start, end = int(value[0]), int(value[1])
    if start < 0 or end <= start or end > int(seq_len):
        raise ValueError("token_span must be a non-empty half-open span within hidden_states.")
    return (start, end)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Train/evaluate a claim-level factuality risk probe")
    parser.add_argument("--records", required=True, help="JSON/JSONL records with claim hidden_states and targets")
    ...
    parser.add_argument(
        "--record-layer",
        type=int,
        help="select one layer from records containing layer_hidden_states/claim_hidden_states_by_layer",
    )
    parser.add_argument(
        "--soft-target-cutoff",
        type=float,
        help=(
            "derive calibration labels from risk_target when hard labels are absent; "
            "records with risk_target > cutoff are treated as anomalous"
        ),
    )
    parser.add_argument("--train-fraction", type=float, default=0.7)
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--layer-idx", type=int)
    parser.add_argument("--pooling", default="mean", choices=("mean", "first_token", "last_token"))
