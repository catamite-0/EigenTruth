...
def build_product_promotion_contract_smoke(
    *,
    ...
    _assert_contract_metadata(metadata)
...
def _assert_contract_metadata(metadata: Mapping[str, Any]) -> None:
    ...
    if _int(metadata.get("promotion_contract_evidence_handoff_present_metric_count")) < 65:
        raise AssertionError("default product promotion handoff lost required metrics.")
    ...
    missing_groups = REQUIRED_HANDOFF_GROUPS.difference(group_statuses)
    if missing_groups:
