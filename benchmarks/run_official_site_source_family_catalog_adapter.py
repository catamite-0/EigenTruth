"""Fetch official-site pages for official source-family catalog tasks.

This adapter consumes the non-evidence collection-task JSONL produced by
``plan_source_family_catalog_collection.py`` plus a URL seed JSONL. It writes
adapter-ready ``source_family=official`` catalog documents for
...
provenance and official-page metadata.
"""
...
def run_official_site_source_family_catalog_adapter(
    *,
    tasks_path: str | Path,
    seeds_path: str | Path,
    ...
    """Fetch seeded official pages and write source-family catalog rows."""
    ...
    matched_seeds = tuple(
        ...
        if _seed_task_id(seed) in tasks_by_id
    )
    if not matched_seeds:
        raise ValueError("no official URL seeds matched the selected collection tasks.")
    ...
    if artifact_manifest_path is not None:
        ...
        artifacts: dict[str, str | Path] = {
            ...
            "official_url_seeds": Path(seeds_path),
            "source_family_catalog": Path(output_path),
        }
        if report_json_path is not None:
            artifacts["official_site_catalog_report"] = Path(report_json_path)
...
def _catalog_row_from_seed_and_page(
    ...
    *,
    ...
    text = _truncate_text(seed_text or extracted_text or title, max_text_chars)
    if not text:
        return None
    domain = _clean(seed.get("domain")) or _url_domain(url)
    provider = _clean(seed.get("provider") or seed.get("source_provider")) or domain or PROVIDER
    ...
    return {
        "text": text,
        "title": title,
        "source": _clean(seed.get("source")) or f"official_site:{_stable_suffix(url)}",
        "url": url,
        "provider": provider,
        "source_family": SOURCE_FAMILY,
...
def _load_seed_rows(path: str | Path) -> tuple[dict[str, Any], ...]:
    ...
    with Path(path).open(encoding="utf-8") as handle:
        for line_no, line in enumerate(handle, start=1):
            ...
            rows.append(dict(payload))
    if not rows:
        raise ValueError("official URL seed file is empty.")
    return tuple(rows)
...
def _default_fetch_text(*, timeout_seconds: float) -> Callable[[str, Mapping[str, str]], str]:
    def fetch(url: str, headers: Mapping[str, str]) -> str:
        ...
        try:
            ...
            raise RuntimeError(f"official site HTTP {exc.code}: {body[:300]}") from exc
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    payload = run_official_site_source_family_catalog_adapter(
    ...
    print(
        "official_site_source_family_catalog_adapter_ok "
        f"status={payload['status']} "
        f"tasks={summary['task_count']} "
        f"seeds={summary['matched_seed_count']} "
