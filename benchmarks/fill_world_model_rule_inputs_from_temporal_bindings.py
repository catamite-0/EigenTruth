"""Fill temporal rule inputs from explicit source-backed bindings.

This workflow is a narrow bridge for temporal consistency rule tasks. It does
not infer claim dates, recover source timestamps, or verify freshness by
itself. Callers must provide JSONL binding rows that explicitly state the claim
time, source time, retrieval time, and source citation. Filled rows remain
adapter inputs and still require deterministic execution plus promotion before
product handoff.
"""
...
TEMPORAL_COLLECTION_FAMILY = "temporal_snapshot_rule_input_collection"
RESERVED_FIELDS = {"answer", "answers", "is_false", "label", "labels", "model_answer", "score_label"}
READY_REVIEW_STATUSES = {"", "ready", "approved"}
DEFAULT_TEMPORAL_RELATION = "source_time_at_or_after_claim_time_and_not_after_retrieval"
DEFAULT_SOURCE_FACT_TYPE = "temporal_snapshot"


def fill_world_model_rule_inputs_from_temporal_bindings(
    *,
    input_tasks: Sequence[Mapping[str, Any]],
    temporal_bindings: Sequence[Mapping[str, Any]],
    metadata: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    """Return filled temporal rule-input rows plus fail-closed diagnostics."""
    ...
    for task in input_tasks:
        ...
        if str(task.get("collection_family") or "") != TEMPORAL_COLLECTION_FAMILY:
            unfilled.append(_unfilled(task, reason="unsupported_collection_family"))
            continue
        binding = bindings_by_request.get(request_id)
        if binding is None:
            unfilled.append(_unfilled(task, reason="missing_temporal_binding"))
            continue
        unused_bindings.discard(request_id)
        failures = _binding_failures(binding, duplicate=request_id in duplicate_binding_ids)
        if failures:
            unfilled.append(_unfilled(task, reason="invalid_temporal_binding", failures=failures))
            continue
        filled.append(_filled_temporal_input(task, binding=binding))
    ...
    return {
        ...
        "scope": (
            "Fills explicit temporal consistency rule inputs from caller-supplied "
        ...
        "label_usage": {
            ...
            "temporal_metadata_bound_to_rule_inputs": True,
            "source_backed_temporal_values_required": True,
        ...
        "skipped_temporal_bindings": skipped_bindings,
...
def run(
    *,
    ...
    temporal_bindings_path: str | Path,
    output_dir: str | Path,
    ...
    """Fill, write, manifest, and optionally register temporal rule-input rows."""
    if registry_path is not None and (not name or not version):
        raise ValueError("registry_path requires name and version.")
    output = Path(output_dir)
    output.mkdir(parents=True, exist_ok=True)
    report_path = Path(report_json_path or output / "rule-input-temporal-binding-fill.json")
    ...
    skipped_path = Path(skipped_bindings_path or output / "skipped-temporal-bindings.jsonl")
    ...
    temporal_bindings = _load_jsonl_mappings(temporal_bindings_path)
    payload = fill_world_model_rule_inputs_from_temporal_bindings(
        input_tasks=input_tasks,
        temporal_bindings=temporal_bindings,
        metadata=metadata,
    )
    ...
    payload["paths"] = {
        ...
        "temporal_bindings": str(temporal_bindings_path),
        "report": str(report_path),
        "rule_inputs": str(inputs_path),
        "unfilled_tasks": str(unfilled_path),
        "skipped_temporal_bindings": str(skipped_path),
    ...
    _write_jsonl(skipped_path, payload["skipped_temporal_bindings"], compact=compact_json)
    manifest = build_artifact_manifest(
        {
            "rule_input_temporal_binding_fill": report_path,
            "rule_inputs": inputs_path,
            "unfilled_rule_input_tasks": unfilled_path,
            "skipped_temporal_bindings": skipped_path,
            "rule_input_tasks": Path(input_tasks_path),
            "temporal_bindings": Path(temporal_bindings_path),
        },
...
def _filled_temporal_input(task: Mapping[str, Any], *, binding: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": 1,
        "workflow": WORKFLOW,
        ...
        "rule_family": str(task.get("rule_family") or "temporal_consistency"),
        ...
        "temporal_relation": _clean(binding.get("temporal_relation")) or DEFAULT_TEMPORAL_RELATION,
        "source_fact_type": _clean(binding.get("source_fact_type")) or DEFAULT_SOURCE_FACT_TYPE,
        ...
        "provenance": {
            ...
            "claim_time_source": str(binding.get("claim_time_source") or ""),
            "source_time_source": str(binding.get("source_time_source") or ""),
            "retrieved_at_source": str(binding.get("retrieved_at_source") or ""),
...
def _binding_failures(binding: Mapping[str, Any], *, duplicate: bool) -> tuple[str, ...]:
    failures = []
    if duplicate:
        failures.append("duplicate_temporal_binding")
    ...
    for key in ("claim_time", "source_time", "retrieved_at"):
        if _clean(binding.get(key)) and _parse_temporal_value(binding.get(key)) is None:
...
def _summary(
    *,
    ...
    source_family_counts = Counter(str(item.get("source_family") or "") for item in filled)
    source_fact_type_counts = Counter(str(item.get("source_fact_type") or "") for item in filled)
    review_status_counts = Counter(str(item.get("review_status") or "") for item in temporal_bindings)
    return {
        "input_task_count": len(input_tasks),
        "temporal_binding_count": len(temporal_bindings),
        ...
        "source_fact_type_counts": _sorted_counter(source_fact_type_counts),
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    parser.add_argument("--temporal-bindings", required=True)
    ...
    print(
        "world_model_rule_input_temporal_binding_fill_ok "
