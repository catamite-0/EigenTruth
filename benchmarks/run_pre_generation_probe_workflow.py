"""Run the pre-generation probe evidence workflow end to end.

This workflow closes the benchmark-side path for soft-target attention probing:

1. optionally run ``eval_truthfulqa.py`` to export pre-generation hidden-state
   records from a forced-answer forward pass;
2. run ``eval_pre_generation_probe.py`` in layer-sweep mode to select a probe
   layer and save the recommended probe plus split-conformal calibration;
3. write a compact workflow report and artifact manifest.
...
EVAL_PRE_GENERATION_PROBE_SCRIPT = Path("benchmarks") / "eval_pre_generation_probe.py"
...
DEFAULT_PRE_GENERATION_LAYERS = (-1, -2)
...
class PreGenerationProbeWorkflowConfig:
    """Configuration for the pre-generation probe evidence workflow."""
    ...
    pre_generation_layers: Sequence[int] = DEFAULT_PRE_GENERATION_LAYERS
    record_grain: str = "candidate"
    offline: bool = True
    ...
    def __post_init__(self) -> None:
        ...
        for field_name in ("records_path", "report_path", "artifact_manifest_path"):
        ...
        object.__setattr__(self, "pre_generation_layers", tuple(int(layer) for layer in self.pre_generation_layers))
        object.__setattr__(
            self,
            "sweep_layers",
        ...
        if self.record_grain not in PRE_GENERATION_RECORD_GRAINS:
            raise ValueError(f"record_grain must be one of: {', '.join(PRE_GENERATION_RECORD_GRAINS)}.")
        if self.best_by not in PROBE_BEST_BY:
            raise ValueError(f"best_by must be one of: {', '.join(PROBE_BEST_BY)}.")
        if not self.pre_generation_layers:
            raise ValueError("pre_generation_layers must not be empty.")
        if self.limit < 0:
    ...
    def truthfulqa_report_path(self) -> Path:
        return self.output_dir / "truthfulqa-pre-generation-export.json"

    @property
    def probe_report_path(self) -> Path:
        return self.output_dir / "pre-generation-probe-layer-sweep.json"

    @property
    def best_probe_path(self) -> Path:
        return self.output_dir / "best-pre-generation-probe.pt"

    @property
    def best_calibration_path(self) -> Path:
        return self.output_dir / "best-pre-generation-calibration.json"

    @property
    def resolved_report_path(self) -> Path:
        if self.report_path is not None:
            return Path(self.report_path)
        return self.output_dir / "pre-generation-probe-workflow.json"
...
def run_pre_generation_probe_workflow(config: PreGenerationProbeWorkflowConfig) -> dict[str, Any]:
    """Run or plan the pre-generation probe workflow."""
    ...
    report = {
        ...
        "evidence_scope": {
            "claim": "pre-generation probe workflow reproducibility",
            "not_a_claim": "model-level hallucination detector superiority",
            "notes": (
                "Small runs validate plumbing and artifact compatibility. Promote detector-quality claims only after "
                "larger held-out multi-model evaluation."
            ),
        },
    }
    ...
    print(
        "pre_generation_probe_workflow_ok "
...
def _truthfulqa_command(config: PreGenerationProbeWorkflowConfig) -> list[str]:
    command = [
        ...
        "--pre-generation-record-grain",
        config.record_grain,
        "--progress-every",
        str(config.progress_every),
        "--json",
        str(config.truthfulqa_report_path),
        "--dump-pre-generation-probe-records",
        str(config.resolved_records_path),
        f"--pre-generation-probe-layers={_comma_ints(config.pre_generation_layers)}",
        "--seed",
        str(config.seed),
    ]
    if config.offline:
...
def _probe_command(config: PreGenerationProbeWorkflowConfig) -> list[str]:
    sweep_layers = "auto" if config.sweep_layers is None else _comma_ints(config.sweep_layers)
    command = [
        str(config.python_executable),
        str(EVAL_PRE_GENERATION_PROBE_SCRIPT),
        "--records",
        str(config.resolved_records_path),
        "--json",
...
def _paths_payload(config: PreGenerationProbeWorkflowConfig) -> dict[str, str]:
    return {
...
def _config_payload(config: PreGenerationProbeWorkflowConfig) -> dict[str, Any]:
    return {
        "model": config.model,
        "dtype": config.dtype,
        "layer": config.layer,
        ...
        "pre_generation_layers": tuple(config.pre_generation_layers),
...
def _workflow_status(*, dry_run: bool, probe_payload: Mapping[str, Any] | None) -> str:
    if dry_run:
        return "planned"
...
def _config_from_args(args: argparse.Namespace) -> PreGenerationProbeWorkflowConfig:
    return PreGenerationProbeWorkflowConfig(
        ...
        pre_generation_layers=_parse_int_list(args.pre_generation_layers) or DEFAULT_PRE_GENERATION_LAYERS,
        record_grain=args.record_grain,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the pre-generation probe evidence workflow")
    ...
    parser.add_argument("--pre-generation-layers", default="-1,-2")
    parser.add_argument("--record-grain", default="candidate", choices=PRE_GENERATION_RECORD_GRAINS)
    ...
    parser.add_argument("--best-by", default="auto", choices=PROBE_BEST_BY)
    ...
    run_pre_generation_probe_workflow(_config_from_args(parser.parse_args()))
