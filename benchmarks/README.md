...
payloads plus the checked-in v1.9 product handoff to verify that direct gates,
cache-profile gates, worker-count sweep decisions, INSIDE sampling
sample-efficiency gates, registry-backed baselines, the default promotion
contract/evidence-handoff path, ProductTrace replay, and release gates pass
acceptable candidates, reject bounded telemetry payloads where full traces are
required, and catch expected regressions. They are stable enough for default
local/CI checks because they do not load a model or measure machine speed. Use
real `eval_truthfulqa.py --profile-json`
